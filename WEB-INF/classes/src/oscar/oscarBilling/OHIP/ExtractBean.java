...
public void dbQuery(String[] dbP){
 ...
 password = dbParam[3];
   try{
      ...
      batchHeader = HE + "B" + ohipVer + getOhipCenter() + output.substring(0,8) + zero(batchOrder) + batchCount + space(6) + groupNo + providerNo + specialty + space(42) + "\r";
     ...
     //   ohipVer = rs.getString("organization_spec_code");
      ...
      // outPatientDate = rs.getDate("visitdate").toString();
        ...
        count = 8 - count;
        hcHin =hin;
        if (hcFlag.compareTo("H")==0){ healthcardCount = healthcardCount + 1;hcHin =hin; hin = space(12); patientHeader2 = "\n" + HE + "R" + hcHin + hcLast + hcFirst + demoSex + hcType + space(47) + "\r"; }else{patientHeader2 = ""; }
        if (visitType == null){
        ...
        invCount = 0;
        query2 = "select * from billingdetail where billing_no='"+ invNo +"' and status='"+ specCode+"'";
 	 ...
 	 while (rs2.next())
      	{
      		...
      		count = 6 - fee.length();
      	        apptDate = appt.substring(0,4) + appt.substring(5,7) + appt.substring(appt.length()-2);
                ...
                BigTotal = BigTotal.add(bdFee);
      	        ...
      	        value = value + "\n" +HE + "T" + serviceCode + space(2) + zero(count) + fee + "0" + billingUnit + apptDate + diagcode + space(12) + space(5) + space(2) + space(6) + space(25) + "\r";
      	        	      if (invCount == 0) {
      	      	htmlContent = htmlContent + "<tr><td class='bodytext'>" + invNo +"</td><td class='bodytext'>" + demoName + "</td><td class='bodytext'>" +hcHin + "</td><td class='bodytext'>"+ apptDate+"</td><td class='bodytext'>"+ serviceCode+"</td><td align='right' class='bodytext'>"+fee+"</td><td align='right' class='bodytext'>"+diagcode+"</td><td class='bodytext'> &nbsp; &nbsp;"+referral+hcFlag+m_Flag+" </td></tr>";
		       		}
		  		else {
		  			htmlContent = htmlContent + "<tr><td class='bodytext'>&nbsp;</td><td class='bodytext'>&nbsp;</td><td class='bodytext'>&nbsp;</td><td class='bodytext'>&nbsp;</td><td class='bodytext'>"+ serviceCode+"</td><td align='right' class='bodytext'>"+fee+"</td><td align='right' class='bodytext'>"+diagcode+"</td><td class='bodytext'>&nbsp;</td></tr>";
		    		}
 		invCount = invCount + 1;
