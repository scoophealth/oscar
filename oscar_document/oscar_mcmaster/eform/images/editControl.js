...
function insertEditControl() {
	...
	if ((agent.indexOf("msie") != -1) && (agent.indexOf("opera") == -1)) { 
		...
		editControlHTML = editControlHTML.replace("[select-template]", "");
	} else {
...
function editControlContents(editorname) {
	...
	if (document.designMode) {
		...
		if (window[editorname]) return window[editorname].document.body.innerHTML;
...
function loadTemplate(selectname){
	...
	//TODO while IE can onload asign a template find some way for changing the iFrame in IE dynamically
  	...
  	if (cursel != 0) { // First one is a label 
		...
		//need to put some delay to ensure that the new src is loaded before we parse it
