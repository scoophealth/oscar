...
import org.caisi.PMmodule.common.PassIntakeFormVars;
...
import org.caisi.PMmodule.utility.DateUtils;
import org.caisi.PMmodule.utility.UtilDateUtilities;
...
throws  IOException, ServletException                             
{
    ...
    Formintakea intakeAClientInfo = null;
    Demographic client = null;
    Formintakea newIntakeA = null;
    boolean addToIntakeAOK = false;
    int numOfIntakeAs = 0;
...
//endDate = "2005-11-30";
		...
		java.util.Date startDateObj = new java.util.Date( Integer.parseInt(startDateArr[0]),
				                                Integer.parseInt(startDateArr[1]),
						                        Integer.parseInt(startDateArr[2]) );
		
		java.util.Date endDateObj = new java.util.Date(Integer.parseInt(endDateArr[0]),
...
private int calculateNumOfReadmission(List intakAs)
{
	...
	while(listIterator.hasNext())
	{
		...
		if( intakeA.getCboxDateofreadmission() != null  && 
			intakeA.getCboxDateofreadmission().equalsIgnoreCase("Y"))
		{
			numOfReadmission++;
		}
	}
...
//################################################################################
private double calculateProportionOfReadmission(List intakeAs)
{//proportion = numOfReadmissions / numOfAllAdmissions
	...
	proportion = (new Integer(numOfReadmission).doubleValue()) /
	             (new Integer(intakeAs.size()).doubleValue()); 
	
	return proportion;
}

//################################################################################
private double calculateFractionOfYear(java.util.Date startDate, java.util.Date endDate)
{// if start date=Jan 1 and end Date = April 1 then 
...
//################################################################################
private double calculateRateOfAdmission(double fractionOfYear, int numOfAdmission)
{// if start date=Jan 1 and end Date = April 1 then 
 //	time period = 4 months or 4/12=0.333 years. 
...
//################################################################################
private double calculateRateOfReadmission(double fractionOfYear, int numOfReadmission)
{// if start date=Jan 1 and end Date = April 1 then 
 //	time period = 4 months or 4/12=0.333 years. 
...
private double calculateAvgDOB(List intakeAs)
{
	...
	while(listIterator.hasNext())
	{
		...
		try
		{
			if( intakeA.getYear() != null     && 
				intakeA.getMonth() != null    &&
				intakeA.getDay() != null      &&
				!intakeA.getYear().equals("")  &&
				!intakeA.getYear().equals("0001")  &&
				!intakeA.getMonth().equals("")  &&
				!intakeA.getDay().equals("")   )
			{
				dob = UtilDateUtilities.calcAge(
						intakeA.getYear(), 
						intakeA.getMonth(), 
						intakeA.getDay());
				totalDOB += dob;
...
//################################################################################
private List calculateNumOfClientsAdmittedInPeriod(
		IntakeAManager intakeAMgr, List intakeAs)
{
	if(intakeAMgr == null  ||  intakeAs == null  ||  intakeAs.size() <= 0)
	{
		return null;
...
}//end of class IntakeAReport1Action
