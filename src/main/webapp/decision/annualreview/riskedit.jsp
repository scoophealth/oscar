...
<%
  ...
  if(request.getParameter("submit")!=null && request.getParameter("submit").compareTo(" Save ")==0) {
    FileWriter inf = new FileWriter(session.getServletContext().getRealPath("/decision/annualreview/desannualreviewplannerrisk.xml"));
...
<%
//		try {
			File file = new File(session.getServletContext().getRealPath("/decision/annualreview/desannualreviewplannerrisk.xml"));
