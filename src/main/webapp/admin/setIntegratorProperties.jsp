...
<style type="text/css">
table#integratorPrefTable {
	border-collapse: separate;
	border-spacing: 0px 4px;
	width: 300px;
}

table#integratorPrefTable tr:nth-child(even) {
	background: #ccc;
}

table#integratorPrefTable td input {
	position: relative;
	left: 30px;
}
...
<table class="MainTable" id="scrollNumber1" name="encounterTable">
	<tr class="MainTableTopRow">
		<td class="MainTableTopRowLeftColumn"><bean:message key="provider.integratorPreferences.preferences" /></td>
		<td style="color: white" class="MainTableTopRowRightColumn"><bean:message key="provider.integratorPreferences.chooseDataSets" /></td>
	</tr>
	<tr>
		...
		<html:form action="/setProviderStaleDate.do">
			...
			<p>During the next Integrator push, send the following patients:<br />
			<input type="radio" name="integrator_full_push" value="1" <%=getChecked(properties[18], true) %> /> All Rostered Patients<br />
			...
			<table id="integratorPrefTable" border=0>
			 ...
			 --%>
			<input type="submit" onclick="$('input[type=radio]').removeAttr('disabled');" value="<bean:message key="provider.integratorPreferences.save" />" />
