...
public class OscarAppointmentDao extends AbstractDao<Appointment> {
	...
	public Appointment findNextAppointment(Integer demographicId) {
		Query query = entityManager.createQuery("FROM Appointment appt WHERE appt.demographicNo = :demographicNo AND BINARY appt.status NOT LIKE '%C%' AND BINARY appt.status NOT LIKE '%D%'" +
