...
public class OLISHL7Handler implements MessageHandler {
	...
	public HashMap<String,String> getPerformingFacilityAddress(int obr) {
		...
		try {
			...
			if ("".equals(identifier)) { return null; }
    ...
    private int getZBXLocation(int i, int j){
        ...
        for (int k = 0; k != segments.length && obxCount <= j && obrCount <= i; k++){
        	...
        	if (segId.equals("OBR")) { obrCount++; }
        	
        	// We count only OBX's for the desired OBR
            else if (segId.equals("OBX") && obrCount == i) { obxCount++; }

        	// Check that this segment is an NTE and we are in the right OBR/OBX position.
            else if (segId.equals("ZBX") && obxCount == j && obrCount == i) { return k; }
        }        
    ...
    public String getTestResultStatusMessage(char status) {
    	switch (status) {
    	case 'C' : return "Amended";
    	case 'F' : return "Final";
    	case 'P' : return "Preliminary";
    	case 'X' : return "Could not obtain results";
    	case 'W' : return "Invalid";
    	case 'Z' : return "Ancillary information";
    	case 'N' : return "Not performed";
    	default  : return "";
    	}
    }
    ...
    public String getTestRequestStatusMessage(char status) { 
    	switch(status) {
    	case 'A': return "Some, but not all, results available";
    	case 'C': return "Correction to results";
    	case 'E': return "OLIS has expired the test request because no activity has occurred within a reasonable amount of time.";
    	case 'F': return "Final results; results stored and verified. Can only be changed with a corrected result.";
    	case 'I': return "No results available; specimen received, procedure incomplete.";
    	case 'O': return "Order received; specimen not yet received. ";
    	case 'P': return "Preliminary: A verified early result is available, final results not yet obtained.";
    	case 'X': return "No results available; Order canceled";
    	default: return "";
    	}
    }
    ...
    public String getOBRComment(int i, int j){
        ...
        try{
            String[] segments = terser.getFinder().getRoot().getNames();
            int k = getNTELocation(i, -1);
            if (j >  0) { k = indexOfNextNTE(segments, k+1, j); }
    ...
    public String getOBRSourceOrganization(int i, int j) {
    	try{
            ...
            if (key == null || key.indexOf(":") == -1) { return ""; }
    ...
    public String getOrganizationType(String ident) {
    	if (ident.equals("2.16.840.1.113883.3.59.1")) { return "Lab"; } 
    	if (ident.equals("2.16.840.1.113883.3.59.2")) { return "SCC"; }
    	if (ident.equals("2.16.840.1.113883.3.59.3")) { return "Hospital"; }
    	return "";
    }
    ...
    public int indexOfNextNTE(String[] segments, int pos, int skip) { 
    	...
    	while (pos < segments.length) {
    		...
    		if (segId.equals("OBR")) { break; }
    		if (segId.equals("OBX")) { break; }
    		if (segId.equals("NTE")) { 
    			count++;
    			if (count >= skip) { return pos; } 
    		}
    		pos++;
    	}
    	return -1;
    ...
    public String getOBXComment(int i, int j, int nteNum){
    	 try{
             String[] segments = terser.getFinder().getRoot().getNames();
             int k = getNTELocation(i, j);
             if (nteNum >  0) { k = indexOfNextNTE(segments, k, nteNum+1); }
             Structure[] nteSegs = terser.getFinder().getRoot().getAll(segments[k]);
             Segment nteSeg = (Segment) nteSegs[0];
    ...
    public String parseFullNameFromSegment(String ident) {
        ...
        try {
			if(terser.get(ident+"3") != null)
			    name = name +" "+ terser.get(ident+"3");
		} catch (HL7Exception e) {
			name = null;
		}
        try {
			if(terser.get(ident+"1") != null)
			    name = name +" "+ terser.get(ident+"1");
		} catch (HL7Exception e) {
			temp = null;
		}
        try {
			if(terser.get(ident+"4")!= null)
			    name = name +" "+ terser.get(ident+"4");
		} catch (HL7Exception e) {
			temp = null;
		}
        try {
			if(terser.get(ident+"6") != null)
			    name = name +" "+ terser.get(ident+"6");
		} catch (HL7Exception e) {
			temp = null;
		}
    ...
    public String getPatientLocation(){
        try{
            ...
            String mailing = String.format("%s %s %s",
						            		getString(terser.get("/.PID-11-3")),
						    				getString(terser.get("/.PID-11-4")),
											getString(terser.get("/.PID-11-5")));
    ...
    private int getNTELocation(int i, int j){
        ...
        for (int k = 0; k != segments.length && obxCount <= j && obrCount <= i; k++){
        	...
        	// Check that this segment is an NTE and we are in the right OBR/OBX position.
            else if (segId.equals("NTE") && obxCount == j && obrCount == i) { return k; }
        }        
        return segments.length - 1;
    }
    ...
    private String getFullDocName(String docSeg) throws HL7Exception{
        ...
        if (terser.get(docSeg+"13") != null) {
        	...
        	if (modifier.equals("MDL")) { modifier = "MD"; }
        	if (modifier.equals("ML")) { modifier = "RM"; }
        	if (modifier.equals("NPL")) { modifier = "RN(EC)"; }
        	if (modifier.equals("DDSL")) { modifier = "DDS"; } 

        }
        if(terser.get(docSeg+"1") != null) {
            docName = docName +" "+ "<span style=\"margin-left:15px; font-size:8px; color:#333333;\">" 
            + modifier +  " " +  terser.get(docSeg+"1") +"</span>";
        }
	...
	private String getOffsetName(String offset) {		
		if (offset.equals("-0400")) { return "EDT"; }
		else if (offset.equals("-0500")) { return "EST"; }
		else if (offset.equals("-0600")) { return "CST"; }
		else if (!offset.trim().equals("")) { return "UTC" + offset; }  
		return "";
	}
