...
public class ConsultationRequestExtDaoTest extends DaoTestFixtures {
	...
	@Test
	/**
	 * Ensures that ConsultationRequestExt records can be deleted
	 * by request id by using the clear() method.
	 * @throws Exception
	 */
	public void testClear() throws Exception {
		...
		// Before running the clear() method, assert that the table
		// contains all newly created ConsultationRequestExts records
		List<ConsultationRequestExt> oldResult = dao.findAll();
		List<ConsultationRequestExt> expectedResult = new ArrayList<ConsultationRequestExt>(Arrays.asList(
				conReqExt1, conReqExt2, conReqExt3
				));
		assertTrue(oldResult.size() == expectedResult.size());
		assertTrue(oldResult.containsAll(expectedResult));
		
		// Clear all ConsultationRequestExt records where the request ID is null
		dao.clear(requestId);
		List<ConsultationRequestExt> result = dao.findAll();
		expectedResult = new ArrayList<ConsultationRequestExt>(Arrays.asList(
				conReqExt2
				));
