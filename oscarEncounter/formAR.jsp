...
function redirect(formId) {
    if(formId==0) {
        window.location="formARPg1.jsp?demographic_no=<%=demoNo%>&formId=<%=formId%>&provNo=<%=provNo%>";
    } else {
        window.location="formAR<%=props.getProperty("c_lastVisited", "Pg1")%>.jsp?demographic_no=<%=demoNo%>&formId=<%=formId%>&provNo=<%=provNo%>";
    }
}
</script>
