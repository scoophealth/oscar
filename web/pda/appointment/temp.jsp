...
%>
<%@ page import="java.lang.*, java.util.*, java.text.*,java.sql.*" errorPage="errorpage.jsp" %>
<jsp:useBean id="mainBean" class="oscar.AppointmentMainBean, oscar.MyDateFormat" scope="session" />
...
<tr>
 <td width="25%" bgcolor="#3EA4E1" align="center"><font face="sans-serif" size="2">
   ...
   <a href="providerapptadmin?year=<%=curYear%>&month=<%=curMonth%>&day=<%=curDay%>&displaymode=month&dboperation=searchappointmentmonth" TITLE='View your template' onmouseover="window.status='View your template';return true">Monthly Template</a></font>
 </td><td width="25%" bgcolor="#3EA4E1" align="center"><font face="sans-serif" size="2">
   <a HREF=# ONCLICK ="popupPage2('pt_searchMAIN.html')" TITLE='Search for patient records' onmouseover="window.status='Search for patient records';return true">Search for Patients</a></font>
 </td><td width="25%" bgcolor="#3EA4E1" align="center"><font face="sans-serif" size="2">
   <a HREF="inet_prov_ENTER_REPORT?ACCESSKEY=17412092&PROVIDER_ID=174" TITLE='Generate a report' onmouseover="window.status='Generate a report';return true">Generate a Report</a></font></td>
</tr>
</table>
</center>
...
<tr>
    ...
    <table BORDER="0" CELLPADDING="0" CELLSPACING="0" height="20">
      <tr>
        ...
        <td></td><td rowspan="2" BGCOLOR="#C0C0C0" ALIGN="MIDDLE" nowrap><font FACE="VERDANA,ARIAL,HELVETICA" SIZE="2">
         <a href="appointmentcontrol.jsp?year=<%=year%>&month=<%=month%>&day=<%=day%>&displaymode=week&dboperation=searchappointmentweek" TITLE='View your daily schedule' OnMouseOver="window.status='View your weekly schedule' ; return true">Week</a></font></td>
        <td></td><td rowspan="2" BGCOLOR="#C0C0C0" ALIGN="MIDDLE" nowrap><font FACE="VERDANA,ARIAL,HELVETICA" SIZE="2">
  ...
  <td align="right" valign="bottom">
   ...
   <INPUT TYPE="text" NAME="year" VALUE="" WIDTH="4" HEIGHT="10" border="0"  size="4" maxlength="4">-
   <INPUT TYPE="text" NAME="month" VALUE="" WIDTH="2" HEIGHT="10" border="0" size="2" maxlength="2">-
   <INPUT TYPE="text" NAME="day" VALUE="" WIDTH="2" HEIGHT="10" border="0" size="2" maxlength="2">
   <INPUT TYPE="hidden" NAME="displaymode" VALUE="week" >
   <INPUT TYPE="hidden" NAME="dboperation" VALUE="searchappointmentweek" >
    ...
    <table BORDER="0" CELLPADDING="0" CELLSPACING="0" WIDTH="100%">
      <tr>
        <td BGCOLOR="ivory" width="33%">
         <a href="appointmentcontrol.jsp?year=<%=year%>&month=<%=month%>&day=<%=(day-7)%>&displaymode=week&dboperation=searchappointmentweek">
         &nbsp;&nbsp;<img src="../images/previous.gif" WIDTH="10" HEIGHT="9" BORDER="0" ALT="View Previous WEEK" vspace="2"></a> 
         <b><span CLASS=title><%=strMonthOfYear%>, Week <%=strDayOfWeekInMonth%></span></b>
         <a href="appointmentcontrol.jsp?year=<%=year%>&month=<%=month%>&day=<%=(day+7)%>&displaymode=week&dboperation=searchappointmentweek">
         <img src="../images/next.gif" WIDTH="10" HEIGHT="9" BORDER="0" ALT="View Next WEEK" vspace="2">&nbsp;&nbsp;</a></td>
      ...
      <tr>
        ...
        <table border="1" cellpadding="2" bgcolor="#486ebd" cellspacing="1" width="98%">
        	<tr>
        ...
        %>
						<td NOWRAP BGCOLOR="#F0F0F0" VALIGN="TOP" ALIGN="MIDDLE" width=13%><img src="../images/clear.gif height="32" width="1" border="0">
							<br><font COLOR="RED" FACE="VERDANA,ARIAL,HELVETICA" SIZE="2">
							<a HREF="/cgi-bin/oicgi.exe/inet_prov_access?option=DAY&DAY=4&MONTH_NUM=2&YEAR=2001&PROVIDER_ID=174&ACCESSKEY=17412092&MONTH_TODAY=2&YEAR_TODAY=2001&DAY_TODAY=7"
							"><font COLOR="BLACK" FACE="VERDANA,ARIAL,HELVETICA" SIZE="2"><%=arrayDayOfWeek[ih]%>Sun.<br><b>4</b></font></a>
        <%
        	}
        %>
						<td BGCOLOR="#FOFOFO" WIDTH="5%">&nbsp;</td>
        ...
        %>
          <tr>
          	<td align="RIGHT" bgcolor="#3EA4E1" NOWRAP WIDTH="5%"><font color="WHITE" face="verdana,arial,helvetica" size="2"><%=(hourCursor+am_pm)%></font></td>
						
						<td BGCOLOR="#486ebd" ROWSPAN="1" width=13% ><a HREF="/cgi-bin/oicgi.exe/inet_prov_access?option=DAY&DAY=4&MONTH_NUM=2&YEAR=2001&PROVIDER_ID=174&ACCESSKEY=17412092"
 						TITLE="VIEW APPT."><img src="../images/clear.gif" border=0 width=100% height=100% ></A></td>

            <td align="RIGHT" bgcolor="#3EA4E1" width="10%" NOWRAP><b><font face="verdana,arial,helvetica" size="2"> 
             <a href=# onClick ="popupPage('addappointment.jsp?year=<%=strYear%>&month=<%=strMonth%>&day=<%=strDay%>&start_time=<%=(hourCursor>9?(""+hourCursor):("0"+hourCursor))+":00"+am_pm %>&end_time=<%=(hourCursor>9?(""+hourCursor):("0"+hourCursor))+":15"+am_pm%>')" 
             title='Click to make an appointment at <%=(hourCursor+":00"+am_pm)%>' onMouseOver="window.status='Click to make an appointment at <%=(hourCursor+":00"+am_pm)%>'; return true" class="adhour">
             <%=(hourCursor+" "+am_pm)%> &nbsp;</a></font></b></td>
				<%
          	while (bFirstTimeRs?rs.next():true) { //if it's not the first time to parse the standard time, should pass it by
          	  ...
          	  bFirstTimeRs=true;
        %>	    
            <td bgcolor="#FDFEC7" rowspan="<%=iRows%>" >
             <a href=# onClick ="popupPage('appointmentcontrol.jsp?year=<%=year%>&month=<%=month%>&day=<%=day%>&start_time=<%=startTime%>&displaymode=edit&dboperation=searchappointment')" title="REASON=<%=reason%>; NOTES =<%=notes%>; STATUS=<%=status%>"> 
             <font size="1.75"><b>>></b>&nbsp;<%=startTime%> - <%=endTime%></font></a><font size="1.75">
             <br>
        <%
				...
				%>
        		 <a href=# onClick ="popupPage('../cpp/cppcontrol.jsp?hin=<%=hin%>&displaymode=edit&dboperation=searchcpp')" title="edit the patient record"><%=name%></a></font></td>
        <% 
        			}
          	}
      ...
      <table BORDER="0" CELLPADDING="0" CELLSPACING="0" WIDTH="100%">
        	...
        	<td BGCOLOR="ivory" width="50%">
         	 <a href="appointmentcontrol.jsp?year=<%=year%>&month=<%=month%>&day=<%=(day-1)%>&displaymode=day&dboperation=searchappointmentday">
...
<!-- End sub page table -->
  ...
  <tr>
    <td><a href="../provider/provideradmin.jsp">&lt;--Back to Provider Admin Page</a></td>
