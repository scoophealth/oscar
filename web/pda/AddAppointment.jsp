...
<%  
  if(session.getValue("user") == null)    response.sendRedirect("logout.jsp");
        ...
        if(!request.getParameter("demographic_no").equals (new String(""))){ 
...
//      String queryString = update appointment set demographic_no=?,end_time=?,name=?, notes=?,reason =?,location=?, resources=?, type=?,style=?,billing =?,status=?,createdatetime=?,creator=?,remarks=? where appointment_no=? "}, //provider_no=? and appointment_date=? and start_time=?";     
        String queryString = "insert into appointment "
                  +" (demographic_no,appointment_date,provider_no,start_time,end_time,name,reason,location,status,type,notes,resources ,creator,createdatetime) values  "
                  +" ("+demographic_no+",'"+appointment_date+"',"+provider_no+",'"+start_time+"','"+end_time+"','"+name+"','"+reason+"','"+location+"','"+status+"','"+type+"','"+notes+"','"+resources+"',"+creator+",'"+createdatetime+"' )";
...
/*
  String curYear = appointment_date.substring(0,4);
  String curMonth = appointment_date.substring(5,7);
  String curDay = appointment_date.substring(8,10);
*/ 
...
//      response.sendRedirect("AddAppointmentSearch.jsp?name="+name+"&appointment_date="+appointment_date+"&start_time="+start_time+"&end_time="+end_time+"&back_to=AddAppointmentForm2");
 
 
    String gotoJsp = "AddAppointmentSearch.jsp?name="+name+"&appointment_date="+appointment_date+"&start_time="+start_time+"&end_time="+end_time;
