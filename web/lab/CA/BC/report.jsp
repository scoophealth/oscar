...
<%
	...
	if(rs.next()){
		boolean signed = (DBHandler.getString(rs,"status")!=null && DBHandler.getString(rs,"status").equalsIgnoreCase("S"));
%>
...
<table width="100%" class="DarkBG">
	<tr>
		...
		<td align="right" class="Text" nowrap><%=(signed? ((DBHandler.getString(rs,"last_name")!=null)? "<b>Signed Off By: </b>" +  DBHandler.getString(rs,"last_name") + ", " + DBHandler.getString(rs,"first_name") : "<b>Signed Off By Provider No.:</b> " + DBHandler.getString(rs,"provider_no")) + " on " + DBHandler.getString(rs,"signed_on") : "" )%>
...
<%
	...
	if(rs.next()){
		...
		calendar.setTime(format.parse(DBHandler.getString(rs,"date_of_birth")));
...
<table width="100%">
	...
	<tr>
		...
		<td class="Text"><%=DBHandler.getString(rs,"patient_name")%></td>
		<td class="Text" align="right">DOB:</td>
		<td class="Text" width="100px"><%=DBHandler.getString(rs,"date_of_birth").substring(0, DBHandler.getString(rs,"date_of_birth").indexOf(" "))%></td>
	</tr>
	<tr>
		<td class="Text">PHN:</td>
		<td class="Text"><%=DBHandler.getString(rs,"external_id")%></td>
	...
	<tr>
		...
		<td class="Text"><%=DBHandler.getString(rs,"sex")%></td>
	</tr>
	<tr>
		<td class="Text">Address:</td>
		<td class="Text" colspan="3"><%=DBHandler.getString(rs,"patient_address").replaceAll("\\\\\\.br\\\\", " ")%></td>
	</tr>
	<tr>
		<td class="Text">Phone:</td>
		<td class="Text"><%=DBHandler.getString(rs,"home_number")%></td>
		<td class="Text"></td>
		<td class="Text"></td>
	</tr>
	<%
...
%>
	...
	<tr>
		...
		<td class="Text" colspan="3"><%=DBHandler.getString(rs,"filler_order_number").substring(0, DBHandler.getString(rs,"filler_order_number").indexOf("-", 3))%></td>
	</tr>
	<tr>
		<td class="Text">Ordered By:</td>
		<td class="Text"><%=DBHandler.getString(rs,"ordering_provider").replaceAll("~", ",<br/>")%></td>
		<td class="Text">Requested On:</td>
		<td class="Text"><%=DBHandler.getString(rs,"requested_date_time")%></td>
	</tr>
	<tr>
		<td class="Text">Copies To:</td>
		<td class="Text"><%=DBHandler.getString(rs,"result_copies_to").replaceAll("~", ",<br/>")%></td>
		<td class="Text">Observed On:</td>
		<td class="Text"><%=DBHandler.getString(rs,"observation_date_time")%></td>
	</tr>
	<%
	}
	rs.close();
...
<table cellspacing="0" cellpadding="0" width="100%">
	...
	String section = "";
	while(rs.next()){
		if(DBHandler.getString(rs,"set_id") == null || rs.getInt("set_id") == 1){
			if(!section.equalsIgnoreCase(DBHandler.getString(rs,"diagnostic_service_sect_id"))){
				section = DBHandler.getString(rs,"diagnostic_service_sect_id");
%>
	<tr>
		<td colspan="7">&nbsp;</td>
	</tr>
	<tr>
		<td class="Section" colspan="7"><%=((DBHandler.getString(rs,"diagnostic_service_sect_id")!=null)? DBHandler.getString(rs,"diagnostic_service_sect_id") : "Other")%></td>
	</tr>
	<%
			}
%>
	...
	<tr>
		<td class="Text" colspan="3"><b>Service Id:</b><%=DBHandler.getString(rs,"universal_service_id").substring(DBHandler.getString(rs,"universal_service_id").indexOf(" "))%></td>
		<td class="Text" nowrap><b>Last Modified:</b><%=DBHandler.getString(rs,"results_report_status_change")%></td>
		<td class="Text" nowrap colspan="3"><b>Result Status:</b><%=(DBHandler.getString(rs,"result_status").equalsIgnoreCase("f")? "Final" : "Pending")%></td>
	</tr>
	<tr>
		<td class="Text" valign="top">Note:</td>
		<td class="Text" colspan="6"><%=DBHandler.getString(rs,"obrnote").replaceAll("\\\\\\.br\\\\", " ")%>&nbsp;</td>
	</tr>
	<%
		}
		if(DBHandler.getString(rs,"set_id") != null){
...
%>
	<tr>
		<td>&nbsp;</td>
		<td class="Text" nowrap class="<%=(other? "LightBG" : "WhiteBG")%>"><%=DBHandler.getString(rs,"observation_identifier").substring(DBHandler.getString(rs,"observation_identifier").indexOf(" "))%></td>
		<td class="Text" nowrap class="<%=(other? "LightBG" : "WhiteBG")%>"><b><%=((DBHandler.getString(rs,"abnormal_flags").toUpperCase().equals("N"))? "&nbsp;" : oscar.Misc.check(DBHandler.getString(rs,"abnormal_flags"), "", "&nbsp;"))%></b></td>
		<td class="Text" class="<%=(other? "LightBG" : "WhiteBG")%>"><%=((DBHandler.getString(rs,"abnormal_flags").toUpperCase().equals("N"))? DBHandler.getString(rs,"observation_results") : "<b>" + DBHandler.getString(rs,"observation_results") + "</b>").replaceAll("\\\\\\.br\\\\", " ")%></td>
		<td class="Text" nowrap class="<%=(other? "LightBG" : "WhiteBG")%>"><%=DBHandler.getString(rs,"reference_range")%></td>
		<td class="Text" nowrap class="<%=(other? "LightBG" : "WhiteBG")%>"><%=DBHandler.getString(rs,"units")%></td>
		<td class="Text" nowrap
			title="<%=DBHandler.getString(rs,"obxnote").replaceAll("\\\\\\.br\\\\", " ")%>"
			class="<%=(other? "LightBG" : "WhiteBG")%>"><%=((DBHandler.getString(rs,"obxnote").length() < 20)? DBHandler.getString(rs,"obxnote") : DBHandler.getString(rs,"obxnote").substring(0, 20)).replaceAll("\\\\\\.br\\\\", " ")%></td>
	</tr>
	<%
		}
		other = !other;
...
%>
	...
	<tr>
		<td colspan="7"><textarea name="notes" rows="7"
			style="width: 100%;"><%=oscar.Misc.check(DBHandler.getString(rs,"notes"), "")%></textarea></td>
	</tr>
	<tr class="LightBG">
