...
YAHOO.widget.AutoComplete=function(G,B,J,C){
    ...
    if(G&&B&&J){
        ...
        this.containerExpandEvent=new YAHOO.util.CustomEvent("containerExpand",this);this.typeAheadEvent=new YAHOO.util.CustomEvent("typeAhead",this);this.itemMouseOverEvent=new YAHOO.util.CustomEvent("itemMouseOver",this);this.itemMouseOutEvent=new YAHOO.util.CustomEvent("itemMouseOut",this);
...
YAHOO.widget.AutoComplete.prototype.sendQuery=function(B){
    //oscarLog("12");
    this._bFocused=true;var A=(this.delimChar)?this._elTextbox.value+B:B;
    this._sendQuery(A);
};
YAHOO.widget.AutoComplete.prototype.snapContainer=function(){
    //oscarLog("13");
    var A=this._elTextbox,B=YAHOO.util.Dom.getXY(A);B[1]+=YAHOO.util.Dom.get(A).offsetHeight+2;YAHOO.util.Dom.setXY(this._elContainer,B);
};
...
YAHOO.widget.AutoComplete.prototype.clearList=function(){
    var B=this._elList.childNodes,A=B.length-1;for(;A>-1;A--){
...
YAHOO.widget.AutoComplete.prototype.filterResults=function(K,M,Q,L){
            ...
            if(YAHOO.lang.isString(F)){
                ...
                if(YAHOO.lang.isArray(F)){
                    ...
                    if(this.responseSchema.fields){
                        //oscarLog("if ee");
                        //oscarLog(this.responseSchema.fields[0].key);
                        //oscarLog(this.responseSchema.fields[0]);
                        //oscarLog(this.responseSchema);
                        //oscarLog(this.responseSchema.fields[1]);
                        //oscarLog(this.responseSchema.fields[2]);
                ...
                //oscarLog("if gg");
                //oscarLog("J="+J);
                //oscarLog("E.indexOf(decodeURIComponent(K))="+E.indexOf(decodeURIComponent(K)));
                //oscarLog("E.toLowerCase().indexOf(decodeURIComponent(K).toLowerCase())="+E.toLowerCase().indexOf(decodeURIComponent(K).toLowerCase()));
                //oscarLog("E="+E+";K="+K);
    ...
    //oscarLog("----end filterResults---");
    return Q;
};
YAHOO.widget.AutoComplete.prototype.handleResponse=function(C,A,B){
    //oscarLog("this._sName="+this._sName);
    //oscarLog(C+"--"+A+"--"+B);
    //oscarLog(A.results+"--"+A.error);
...
YAHOO.widget.AutoComplete.prototype._initContainerHelperEls=function(){
    if(this.useShadow&&!this._elShadow){
        ...
        var B=document.createElement("iframe");B.src=this._iFrameSrc;B.frameBorder=0;B.scrolling="no";B.style.position="absolute";B.style.width=0;B.style.height=0;B.style.padding=0;B.tabIndex=-1;B.role="presentation";B.title="Presentational iframe shim";this._elIFrame=this._elContainer.appendChild(B);
    }
};
...
YAHOO.widget.AutoComplete.prototype._initListEl=function(){
    ...
    while(A.childNodes.length<C){
        B=document.createElement("li");B.style.display="none";
        B._nItemIndex=A.childNodes.length;A.appendChild(B);
    }if(!this._elList){
...
YAHOO.widget.AutoComplete.prototype._sendQuery=function(D){
    ...
    if(this.minQueryLength<0){
        ...
        //oscarLog("if3");
        //oscarLog(this._nDelayID);
        if(this._nDelayID!=-1){
            //oscarLog("clearTimeout");
...
YAHOO.widget.AutoComplete.prototype._populateList=function(K,F,C){
    ...
    //oscarLog(!F.error);
    if(H&&!F.error){
        //oscarLog("before fire");
        this.dataReturnEvent.fire(this,K,F.results);
        //oscarLog(this._bFocused);
        if(this._bFocused){
            var M=decodeURIComponent(K);
            //oscarLog("M="+M);
            this._sCurQuery=M;
            this._bItemSelected=false;            
            //oscarLog(this.maxResultsDisplayed);
            //oscarLog(F);
            //oscarLog(F.results);
            var R=F.results,
            ...
            if(A>0){
                ...
                for(var Q=A-1;Q>=0;Q--){
                    ...
                    if(this.resultTypeList){
                        var B=[];B[0]=(YAHOO.lang.isString(E))?E:E[J]||E[this.key];
                ...
                //oscarLog("containerPopulateEvent fire");
                //oscarLog(this);
                //oscarLog(K);
                //oscarLog(R);
                this.containerPopulateEvent.fire(this,K,R);
                if(this.autoHighlight){
                    var D=this._elList.firstChild;this._toggleHighlight(D,"to");
                    //oscarLog("itemArrowToEvent fire");
                    //oscarLog(this);
                    //oscarLog(D);
                ...
                H=this._doBeforeExpandContainer(this._elTextbox,this._elContainer,K,R);this._toggleContainer(H);
            }else{
                this._toggleContainer(false);
            }return;
        }
    ...
    var A=this.prehighlightClassName;if(this._elCurPrehighlightItem){
        YAHOO.util.Dom.removeClass(this._elCurPrehighlightItem,A);
    }if(B==this._elCurListItem){
        return;
    }if((C=="mouseover")&&A){
        YAHOO.util.Dom.addClass(B,A);this._elCurPrehighlightItem=B;
    }else{
    ...
    this._bItemSelected=true;this._updateValue(A);this._sPastSelections=this._elTextbox.value;this._clearInterval();this.itemSelectEvent.fire(this,A,A._oResultData);this._toggleContainer(false);
