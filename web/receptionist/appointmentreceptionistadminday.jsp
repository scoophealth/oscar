    ...
    <table BORDER="0" CELLPADDING="0" CELLSPACING="0" height="20">
      <tr>
		...
		<caisi:isModuleLoad moduleName="ticklerplus">
         <a href=# onClick ="popupPage(200,680,'receptionistpreference.jsp?provider_no=<%=curUser_no%>&start_hour=<%=startHour%>&end_hour=<%=endHour%>&every_min=<%=everyMin%>&mygroup_no=<%=mygroupno%>&new_tickler_warning_window=<%=newticklerwarningwindow%>');return false;"><bean:message key="receptionist.appointmentreceptionistadminday.btnPref"/></a></font></td>
		</caisi:IsModuleLoad>
...
notes: <%=Misc.htmlEscape(notes)%>" >
            ...
            <% if (vLocale.getCountry().equals("BR")) { %> 
            <a href=# onClick="popupPage2('../demographic/demographiccontrol.jsp?demographic_no=<%=demographic_no%>&displaymode=edit&dboperation=search_detail_ptbr');return false;" title='<bean:message key="receptionist.appointmentreceptionistadminday.masterFile"/>'> | <bean:message key="provider.appointmentProviderAdminDay.btnM"/> </a>
            <%}else{%> 
            <a href=# onClick="popupPage2('../demographic/demographiccontrol.jsp?demographic_no=<%=demographic_no%>&displaymode=edit&dboperation=search_detail');return false;" title='<bean:message key="receptionist.appointmentreceptionistadminday.masterFile"/>'> | <bean:message key="provider.appointmentProviderAdminDay.btnM"/> </a>
	    <%}%>
<% } %>
        		</font></td>
<% 
