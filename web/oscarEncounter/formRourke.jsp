...
function redirect(formId)
{
    if(formId==0)
    {
        window.location="formRourke1.jsp?demographic_no=<%=demoNo%>&formId=<%=formId%>&provNo=<%=provNo%>";
    }
    else
    {
        window.location="formRourke<%=props.getProperty("c_lastVisited", "1")%>.jsp?demographic_no=<%=demoNo%>&formId=<%=formId%>&provNo=<%=provNo%>";
    }
}
</script>
