			...
			<tr>
				...
				<th valign="bottom" class="Header">Loinc Code</th>
                        ...
                        for(String s:list){
                            ...
                            List<Hashtable> codesHash = map.getMappedCodesFromLoincCodes(s);
                            String desc = "";
                            if (codesHash.size() > 0 ){
                                desc = getDesc(codesHash.get(0));
                            }
                            String mappings = getCodeMap(codesHash);  
                            Hashtable<String,Hashtable> h = map.getMappedCodesFromLoincCodesHash(s);
                        ...
                        <tfoot>
                            ...
                            <tr>
                                ...
                                <td class="Cell" >
                                    <ul>
                                    <%
                                    ArrayList<Hashtable> unList = map.getUnmappedMeasurements(type);
                                    for (Hashtable h:unList){
                                    %>
                                       <li><%=h.get(("name"))%></li>
                                    <%}%>
                                    </ul>
...
<%!
  ...
  String getDisplay(Hashtable<String,Hashtable> h, String type){
      Hashtable data = h.get(type);
      if (data == null ){ return "&nbsp;";}
      return data.get("name")+": "+data.get("ident_code");
  }
  
  String getCodeMap(List<Hashtable> list){
