...
<%
  ...
  dbQueries=new String[][] {
    {"provider_add_record", "insert into provider (provider_no,last_name,first_name,provider_type,specialty,team,sex,dob,address,phone,work_phone,ohip_no,rma_no,billing_no,hso_no,status,comments,provider_activity,practitionerNo) values(?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?,?,?,?,?)" },
    ...
    {"provider_update_record", "update provider set last_name=?,first_name=?, provider_type=?, specialty=?,team=?,sex =?,dob=?, address=?,phone=?,work_phone=?,ohip_no =?,rma_no=?,billing_no=?,hso_no=?,status=?, comments=?, provider_activity = ?, practitionerNo = ? where provider_no=? and provider_no!='super'"},
