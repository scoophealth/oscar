...
<link rel="stylesheet" type="text/css"
	href="../../oscarMessenger/encounterStyles.css">
<title><%-- bean:message key="indivoMessenger.DisplayMessages.title"/ --%>
myOSCAR</title>
...
<table class="MainTable" id="scrollNumber1" name="encounterTable">
				...
				<table cellspacing=3>
					<tr>
						...
						<table class=messButtonsA cellspacing=0 cellpadding=3>
							<tr>
								<td
									class="messengerButtonsA<%if (pageMethod.equals("viewMessages")) {%>Current<%}%>">
						...
						<table class=messButtonsA cellspacing=0 cellpadding=3>
							<tr>
								<td
									class="messengerButtonsA<%if (phrActionDAO.ifActionsWithErrors(providerNo)) {%>Warning<%} else if (pageMethod.equals("viewSentMessages")) {%>Current<%}%>">
						...
						<table class=messButtonsA cellspacing=0 cellpadding=3>
							<tr>
								<td
									class="messengerButtonsA<%if (pageMethod.equals("viewArchivedMessages")) {%>Current<%}%>">
						...
						<table class=messButtonsA cellspacing=0 cellpadding=3>
							<tr>
								<td
									class="messengerButtonsA<%if (pageMethod.equals("viewArchivedMessages")) {%>Current<%}%>">
								<a
									href="../../phrExchange.do?method=setExchangeTimeNow&forwardto=phr/msg/DisplayPHRMessages.jsp?method=<%=pageMethod%>"
									class="messengerButtons"> Send & Receive </a></td>
							</tr>
						</table>
						</td>
						<td>
						<table class=messButtonsA cellspacing=0 cellpadding=3>
							<tr>
								<td class="messengerButtonsA"><a
									href="javascript:window.close()" class="messengerButtons">Exit</a>
								</td>
							</tr>
						</table>
						</td>
						...
						<logic:present name="<%=PHRAuthentication.SESSION_PHR_AUTH%>">
							...
							<div>Status: <b>Logged in as <%=providerName%></b> (<%=phrAuth.getUserId()%>)
							<form action="../../phr/Logout.do" name="phrLogout" method="POST"
								style="margin: 0px; padding: 0px;"><input type="hidden"
								name="forwardto"
								value="<%=request.getServletPath()%>?method=<%=(String) request.getParameter("method")%>">
							<center><a
						...
						<logic:notPresent name="<%=PHRAuthentication.SESSION_PHR_AUTH%>">
							...
							<form action="../../phr/Login.do" name="phrLogin" method="POST"
								style="margin-bottom: 0px;"><%--=request.getParameter("phrUserLoginErrorMsg")%>
							...
							<%=providerName%> password: <input type="password"
								id="phrPassword" name="phrPassword"
	...
	<tr>
		<td class="MainTableLeftColumn">&nbsp;</td>
		<td class="MainTableRightColumn"><%-- Sharing approval alerts -------------- --%>
		<%if (actionsPendingApproval != null) {
                    for (PHRAction actionPendingApproval: actionsPendingApproval) {
                        ...
                        String permissionReadable = permission.substring(permission.lastIndexOf(':')+1, permission.length());
                                %>
		<div class="sharingAlert"><span style="float: left;"> Add
		sharing for <span style="font-weight: bold; color: #339a8a;"><%=demographicIndivoId%></span>.
		Share type: <span style="font-weight: bold; color: #d0722e;"><%=permissionReadable%></span>
		(Proposed by <span style="font-weight: bold; color: #339a8a;"><%= senderProvider.getFirst_name()%>
		<%=senderProvider.getLast_name()%></span>) </span> <span style="float: right;"><a
			href="../../phr/UserManagement.do?method=approveAction&actionId=<%=actionPendingApproval.getId()%>"><b>Approve</b></a>
		<a
		...
		<table class="messageTable" border="0" width="99%" cellspacing="1">
			<tr>
				<th bgcolor="#DDDDFF" width="30">&nbsp;</th>
				<th align="left" bgcolor="#DDDDFF"><html-el:link
					action="/phr/PhrMessage?orderby=0">
					<bean:message key="oscarMessenger.DisplayMessages.msgStatus" />
				</html-el:link></th>
				<th align="left" bgcolor="#DDDDFF"><html-el:link
					action="/phr/PhrMessage?orderby=1">
				...
				<th align="left" bgcolor="#DDDDFF"><html-el:link
					action="/phr/PhrMessage?orderby=2">
					<bean:message key="oscarMessenger.DisplayMessages.msgSubject" />
				</html-el:link></th>
				<th align="left" bgcolor="#DDDDFF"><html-el:link
					action="/phr/PhrMessage?orderby=3">
			...
			<c:forEach var="otherAction" items="${indivoOtherActions}">
				<tr
					...
					<td><c:choose>
						<c:when
							test="${otherAction.phrClassification == TYPE_ACCESSPOLICIES}">
						...
						<c:otherwise>
							<c:out value="${otherAction.receiverPhr}" />
						</c:otherwise>
					</c:choose></td>
					<td><c:choose>
						...
						<c:when
							test="${otherAction.phrClassification == TYPE_ACCESSPOLICIES}"><-- Sharing Permission</c:when>
						<c:otherwise><-- Unknown></c:otherwise>
					</c:choose></td>
					<td><fmt:formatDate value="${otherAction.dateQueued}"
						type="DATE" pattern="yyyy-MM-dd HH:mm:ss" /></td>
					<td><c:choose>
						<c:when test="${otherAction.status == STATUS_NOT_AUTHORIZED}">
							<html-el:link
								action="/phr/PhrMessage?&method=resend&id=${otherAction.id}">Resend</html-el:link>
						</c:when>
						<c:otherwise>&nbsp;</c:otherwise>
					</c:choose></td>
					<td><html-el:link
						action="/phr/PhrMessage?&method=delete&id=${otherAction.id}">Remove</html-el:link>
					</td>
				</tr>
			</c:forEach>
			...
			<c:forEach var="actionMessage" items="${indivoMessageActions}">
				<tr
					...
					<td><c:out value="${actionMessage.phrMessage.receiverPhr}" /></td>
					<td><html-el:link
						action="/phr/PhrMessage?method=read&source=actions&comingfrom=viewSentMessages&noreply=yes&id=${actionMessage.id}">
						<c:out value="${actionMessage.phrMessage.docSubject}" />
					</html-el:link></td>
					<td><fmt:formatDate value="${actionMessage.dateQueued}"
						type="DATE" pattern="yyyy-MM-dd HH:mm:ss" /></td>
					<td><c:choose>
						<c:when test="${actionMessage.status == STATUS_NOT_AUTHORIZED}">
							<html-el:link
								action="/phr/PhrMessage?&method=resend&id=${actionMessage.id}">Resend</html-el:link>
						</c:when>
						<c:otherwise>&nbsp;</c:otherwise>
					</c:choose></td>
					<td><html-el:link
						action="/phr/PhrMessage?&method=delete&id=${actionMessage.id}">Remove</html-el:link>
					</td>
				</tr>
			</c:forEach>
			...
			<c:forEach var="sMessage" items="${indivoSentMessages}">
				<tr class="normal">
					<td width="30">&nbsp;</td>
					<td width="75">Sent</td>
					<td><c:out value="${sMessage.receiverPhr}" /></td>
					<td><html-el:link
						action="/phr/PhrMessage?method=read&noreply=yes&comingfrom=viewSentMessages&id=${sMessage.id}">
						<c:out value="${sMessage.docSubject}" />
					</html-el:link></td>
					<td><fmt:formatDate value="${sMessage.dateSent}" type="DATE"
			...
			<c:forEach var="archivedMessage" items="${indivoArchivedMessages}">
				<tr class="normal">
					<td width="30"><c:if test="${archivedMessage.replied}"> --> </c:if>
					...
					<td><c:out value="${archivedMessage.senderPhr}" /></td>
					<td><html-el:link
						action="/phr/PhrMessage?&method=read&comingfrom=viewArchivedMessages&id=${archivedMessage.id}">
						<c:out value="${archivedMessage.docSubject}" />
					</html-el:link></td>
					<td><fmt:formatDate value="${archivedMessage.dateSent}"
						type="DATE" pattern="yyyy-MM-dd HH:mm:ss" /></td>
					<td><html-el:link
						action="/phr/PhrMessage?&method=unarchive&id=${archivedMessage.id}">
                                   Unarchive
                                </html-el:link></td>
				</tr>
			</c:forEach>
			...
			<nested:notEmpty name="indivoMessages">
				...
				<nested:iterate indexId="msgIdx" id="iMessage" name="indivoMessages"
					type="org.oscarehr.phr.model.PHRDocument">
					<tr
						<c:choose>
                                <c:when test="${iMessage.read}">class="normal"</c:when>
                                <c:otherwise>class="new"</c:otherwise>
                            </c:choose>>
						<td bgcolor="#EEEEFF" width="30"><c:if
							test="${iMessage.replied}"> --> </c:if></td>
						<td bgcolor="#EEEEFF" width="75"><c:choose>
							<c:when test="${iMessage.read}">read</c:when>
							<c:otherwise>new</c:otherwise>
						</c:choose></td>
						<td bgcolor="#EEEEFF"><a
							href="javascript: function myFunction() {return false; }"
							onClick="gotoEchart(<c:out value="${iMessage.senderDemographicNo}"/>, '<%=URLEncoder.encode(phrMsgs.get(msgIdx).getBody().replaceAll("\n","<br>"))%>');">
						<c:out value="${iMessage.senderPhr}" /> </a></td>
						<td bgcolor="#EEEEFF"><html-el:link
							action="/phr/PhrMessage?&method=read&comingfrom=viewMessages&id=${iMessage.id}">
							<c:out value="${iMessage.docSubject}" />
						</html-el:link></td>
						<td bgcolor="#EEEEFF"><fmt:formatDate
							value="${iMessage.dateSent}" type="DATE"
							pattern="yyyy-MM-dd HH:mm:ss" /></td>
						<td><html-el:link
							action="/phr/PhrMessage?&method=archive&id=${iMessage.id}">
                                   Archive
                                </html-el:link></td>
					</tr>
				</nested:iterate>
