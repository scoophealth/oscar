...
<phr:IfNotPHRAuthenticated>
	<jsp:include page="../AuthInclude.jsp">
		<jsp:param name="forwardto"
			value="<%="indivo/RegisterIndivo.jsp?demographicNo=" + demographicNo%>" />
		<jsp:param name="pathtophr" value="../" />
	</jsp:include>
</phr:IfNotPHRAuthenticated>
<html-el:form action="/phr/UserManagement" styleId="registrationForm"
	method="POST">
	...
	<div class="objectBlueHeader">Login Information</div>
	<table>
		<tr>
			<td>Username:</td>
			<td><html-el:text styleId="username" property="username"
				value="" size="30" /></td>
		</tr>
		<tr>
			<td>Password:</td>
			<td><html-el:password styleId="password" property="password"
				value="" /></td>
		</tr>
		<tr>
			<td>Repeat Password: <br />
			(min 8 char)</td>
			<td><html-el:password styleId="passwordRepeat"
				property="passwordRepeat" value="" /></td>
		</tr>
	</table>
	</div>
	<div class="objectBlue">
	<div class="objectBlueHeader">Personal Information</div>
	<table>
		<html-el:hidden property="role" value="patient" />
		<tr>
			<td>Name (First, Last):</td>
			<td><html-el:text property="firstName"
				value="${demographic.firstName}" size="20" /> <html-el:text
				property="lastName" value="${demographic.lastName}" size="20" /></td>
		</tr>
		<tr>
			<td>Street Address</td>
			<td><html-el:text property="address"
				value="${demographic.address}" size="40" /></td>
		</tr>
		<tr>
			<td>City</td>
			<td><html-el:text property="city" value="${demographic.city}"
				size="20" /></td>
		</tr>
		<tr>
			<td>Province</td>
			<td><html-el:text property="province"
				value="${demographic.province}" size="20" /></td>
		</tr>
		<tr>
			<td>Postal Code</td>
			<td><html-el:text property="postal"
				value="${demographic.postal}" size="10" /></td>
		</tr>
		<tr>
			<td>Home Phone Number:</td>
			<td><html-el:text property="phone"
				value="${demographic.phone}${demographicHomeExt}" size="20" /></td>
		</tr>
		<tr>
			<td>Work Phone Number:</td>
			<td><html-el:text property="phone2"
				value="${demographic.phone2}${demographicWorkExt}" size="20" /></td>
		</tr>
		<tr>
			<td>E-mail</td>
			<td><html-el:text property="email" value="${demographic.email}"
				size="30" /></td>
		</tr>
		<tr>
			<td>Date of Birth</td>
			<td><html-el:text property="dob"
				value="<%=demographic.getDob("/")%>" size="10" /></td>
		</tr>
	</table>
	</div>
	<div class="objectBlue">
	<div class="objectBlueHeader">Grant Access Rights:</div>
	<%ArrayList<String> providerNums = (ArrayList<String>) ProviderMyOscarIdData.listMyOscarProviderNums();
                    for (int i=0; i<providerNums.size(); i++) {
                        ...
                        if (pd.getFirst_name() == null) continue; %> <span
		style="white-space: nowrap; width: 40px;"><input
		type="checkbox" name="list:grantProviders" value="<%=curProviderNo%>" /><%=pd.getFirst_name()%>
	<%=pd.getLast_name()%></span>&nbsp; &nbsp; <%}%>
	</div>
	<div class="objectBlue">
	<div class="objectBlueHeader">Submit</div>
	<center><html-el:button onclick="validateNSubmit();"
		property="submitButton" styleId="submitButton" value="Submit" /><html-el:button
		property="closeButton" styleId="closeButton" value="Close"
		onclick="window.close();" /></center>
	</div>
</html-el:form>
<div id="submittedMessage" class="objectGreen"
	style="position: absolute; width: 50%; height: 20%; display: none; top: 40%; left: 25%; text-align: center; font-size: 12px;">
<div class="objectGreenHeader">Creating a new user</div>
<br />
Creating a new user...... <br />
<b>Do not close this window.</b></div>
<script type="text/javascript" language="JavaScript">
           enableSubmit();
       </script>
<phr:IfNotPHRAuthenticated>
