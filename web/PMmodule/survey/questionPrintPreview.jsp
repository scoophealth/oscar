...
<c:set var="id" scope="page"><c:out value="${question.id}"/></c:set>
<%
		...
		<c:when test="${question.type.openEnded != null}">
			<c:choose>
				<c:when test="${question.type.openEnded.rows == 1}">
					...
					<td><html-el:text property="data.value(${pageNumber}_${sectionId}_${question.id})" size="90"/></td>
				</c:when>
				<c:otherwise>
					<td colspan="2">
						<c:if test="${question.bold eq 'true'}"><b></c:if>
						...
						<br/>
						<html-el:textarea property="data.value(${pageNumber}_${sectionId}_${question.id})" rows="5" cols="90"></html-el:textarea>
