...
<html:form action="/PMmodule/GenericIntake/Search"
...
%>
	...
	<table id="genericIntakeSearch" width="50%">
		...
		<tr>
			<th>Birth Date</th>
			<td><html:select property="monthOfBirth">
				<html:optionsCollection property="months" value="value"
					label="label" />
			</html:select>&nbsp; <html:select property="dayOfBirth">
				<html:optionsCollection property="days" value="value" label="label" />
			</html:select>&nbsp; <html:text property="yearOfBirth" size="4" maxlength="4" />&nbsp;(YYYY)
			</td>
		</tr>
		<tr>
			<th>Gender</th>
			<td><html-el:select property="gender">
		...
		<caisi:isModuleLoad moduleName="TORONTO_RFQ" reverse="true">
			<caisi:isModuleLoad moduleName="GET_OHIP_INFO" reverse="false">
				<tr>
					<th>Health Card</th>
					<td><html:text property="healthCardNumber" size="10"
						maxlength="10" />&nbsp; <html:text property="healthCardVersion"
						size="2" maxlength="2" />&nbsp;(version)</td>
				</tr>
			</caisi:isModuleLoad>
		</caisi:isModuleLoad>
		<tr>
			<td colspan="2"><br />
			<html:submit onclick="search()">Search</html:submit></td>
		</tr>
	</table>

	<c:if
		test="${not empty requestScope.genericIntakeSearchForm.localMatches}">
		<h5>Local matches</h5>
		<p>The following possible matches were found in the system.</p>
		<p>If your client is not one of these matches, choose the 'New
		Client' button. Otherwise, you can update the existing client file.</p>

		<display-el:table class="simple"
			name="requestScope.genericIntakeSearchForm.localMatches" uid="client"
			...
			<display-el:column property="formattedName" sortable="true"
				title="Name" />
			<display-el:column property="formattedDob" sortable="true"
				title="Date of Birth" />
	...
	%>
		...
		<p>The following possible matches were found in the integrated
		community.</p>
		<display-el:table class="simple" name="requestScope.remoteMatches"
			id="x">
			<%
   		...
   		%>
			...
			<display-el:column title="">
				<input type="submit" value="Copy to Local"
					onclick="copyRemote(<%=facilityId%>,<%=cdi.getFacilityIdIntegerCompositePk().getCaisiItemId()%>)" />
			</display-el:column>
			<display-el:column title="Facility Name">
				<%=facilityName%>
			</display-el:column>
			<display-el:column title="Client Name">
				<c:out value="${x.cachedDemographic.lastName}" />, <c:out
					value="${x.cachedDemographic.firstName}" />
			</display-el:column>
			<display-el:column title="BirthDate">
				<c:out value="${x.cachedDemographic.birthDate.year}" />-<c:out
					value="${x.cachedDemographic.birthDate.month}" />-<c:out
					value="${x.cachedDemographic.birthDate.day}" />
			</display-el:column>
			<display-el:column property="cachedDemographic.gender" title="Gender" />
			<display-el:column property="cachedDemographic.hin"
				title="Health Number" />
