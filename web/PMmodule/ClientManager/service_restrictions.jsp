...
<display:table class="simple" cellspacing="2" cellpadding="3"
	id="service_restriction" name="serviceRestrictions" export="false"
	pagesize="0" requestURI="/PMmodule/ClientManager.do">
	<%
...
%>
	...
	<display:column property="program.name" sortable="true"
		title="Program Name" />
	...
	<display:column sortable="true" title="">
		<input type="button"
			...
			value="Terminate Early" onclick="terminateEarly(<%=temp.getId()%>)" />
	</display:column>
</display:table>
<br />
<br />
...
<table cellpadding="3" cellspacing="0" border="0">
	<tr>
		<th title="Programs">Find a program to add a service restriction
		to...</th>
	</tr>
</table>
</div>
...
<c:if test="${requestScope.do_restrict != null}">
	<c:choose>
			...
			<table width="100%" border="1" cellspacing="2" cellpadding="3">
				...
				<tr class="b">
					<td colspan="2"><input type="button"
						value="Add Service Restriction" onclick="do_service_restriction()" />
		...
		<c:otherwise>
			<b style="color: red">You do not have permission to apply service
			restrictions to "<c:out value="${program.name}" />."</b>
		</c:otherwise>
	</c:choose>
</c:if>
