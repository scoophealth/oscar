...
<display:table class="simple" cellspacing="2" cellpadding="3"
	...
	<display:setProperty name="basic.msg.empty_list"
		value="No staff currently assigned to this program." />
	<display:column sortable="false" title="">
		<a onclick="deleteProvider('<c:out value="${provider.id}"/>');"
			...
			<c:forEach var="team" items="${provider.teams}">
				<tr>
					...
					<td><a href="javascript:void(0);return false;"
						onclick="removeTeam('<c:out value="${provider.id}"/>','<c:out value="${team.id}"/>')">Remove</a>
				</tr>
			</c:forEach>
		</table>
		<select name="x"
			onchange="assignTeam('<c:out value="${provider.id}"/>',this)">
	...
	<display:column sortable="false" title="CAISI Role">
		<select name="x"
			onchange="assignRole('<c:out value="${provider.id}"/>',this);">
...
<table width="100%" border="1" cellspacing="2" cellpadding="3">
	<tr class="b">
		...
		<td><html:hidden property="provider.id" /> <html:hidden
			property="provider.providerNo" /> <%
