...
<html:form action="/Lookup/LookupList.do">
	...
	<table width="100%" border="0">
		<tr>
			<td width="80%">Description: <html:text property="keywordName"
				style="width:100%;" /></td>
			<td width="20%"><html:submit property="method" value="search" /></td>
		</tr>
		<tr>
			<td colspan="2"><display:table class="simple"
				style="width:100%;" cellspacing="0" cellpadding="0" id="lookup"
				...
				<display:column sortable="false" title="Description">
					<a
						href='javascript:selectMe("<c:out value="${lookup.code}" />", "<c:out value="${lookup.description}" />", 
        "<c:out value="${lookupListForm.openerFormName}" />", 
        "<c:out value="${lookupListForm.openerCodeElementName}" />", 
        "<c:out value="${lookupListForm.openerDescElementName}" />");'><c:out
						value="${lookup.description}" /> </a>
				</display:column>
