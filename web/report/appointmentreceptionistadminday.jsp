...
var page = "" + varpage;
windowprops = "height=550,width=700,location=no,"
...
function findProvider(p,m,d) {
  popupPage(200,400, "receptionistfindprovider.jsp?pyear=" +p+ "&pmonth=" +m+ "&pday=" +d+ "&providername="+ document.findprovider.providername.value );
}
//-->
</SCRIPT>
  ...
  <td VALIGN="BOTTOM" HEIGHT="20"> 
    ...
    <table BORDER="0" CELLPADDING="0" CELLSPACING="0" height="20">
      <tr>
        ...
        <td></td><td rowspan="2" BGCOLOR="#C0C0C0" ALIGN="MIDDLE" nowrap><font FACE="VERDANA,ARIAL,HELVETICA" SIZE="2">
         <a HREF="#" ONCLICK ="popupPage2('../billing/billingreport.jsp');return false;" TITLE='Generate a billing report' onmouseover="window.status='Generate a billing report';return true">Billing</a></font></td>
<!--oscarMessenger Code Block-->
        <td></td><td rowspan="2" BGCOLOR="#C0C0C0" ALIGN="MIDDLE" nowrap><font FACE="VERDANA,ARIAL,HELVETICA" SIZE="2">
         ...
         <jaymessage:newMessages providerNo="<%=curUser_no%>"/></a></font></td>
<!--/oscarMessenger code block -->
        <td></td><td rowspan="2" BGCOLOR="#C0C0C0" ALIGN="MIDDLE" nowrap><font FACE="VERDANA,ARIAL,HELVETICA" SIZE="2">
...
<% } else { %>         
         <!--a href="../schedule/scheduleflipview.jsp?startDate=<%=year+"-"+month+"-"+day%>" title="Flip View">Flip View</a> &nbsp;|&nbsp; --> 
            ...
            <td width='1%' <%=DateTimeCodeBean.get("color"+hourmin.toString())!=null?("bgcolor="+DateTimeCodeBean.get("color"+hourmin.toString()) ):("bgcolor="+bgcolordef) %> title='<%=DateTimeCodeBean.get("description"+hourmin.toString())%>'><font color='<%=(DateTimeCodeBean.get("color"+hourmin.toString())!=null && !DateTimeCodeBean.get("color"+hourmin.toString()).equals(bgcolordef) )?"black":"black" %>'><%=hourmin.toString() %></font>
          	...
          	while (bFirstTimeRs?rs.next():true) { //if it's not the first time to parse the standard time, should pass it by
          	  ...
          	  if( (ih > iE*60+iEm) ) { //appt before this time slot (both start&end), get to the next period
         	    ...
         	    //iRows=(iE*60-Math.max(iS*60,ih))/depth+iEm/depth-(Math.max(iS*60,ih)%60)/depth+1; //to see if the period across an hour period
          	  ...
          	  //System.out.println("line: "+bFirstTimeRs);
          	  bFirstTimeRs=true;
        %>	    
            ...
            %>
            <a href="receptionistcontrol.jsp?appointment_no=<%=rs.getString("appointment_no")%>&provider_no=<%=curProvider_no[nProvider]%>&status=<%=status.replace('T',' ').trim()%>&statusch=H&year=<%=year%>&month=<%=month%>&day=<%=day%>&view=<%=view==0?"0":("1&curProvider="+request.getParameter("curProvider")+"&curProviderName="+request.getParameter("curProviderName") )%>&displaymode=addstatus&dboperation=updateapptstatus&viewall=<%=request.getParameter("viewall")==null?"0":(request.getParameter("viewall"))%>"; title="Star Bill" >
...
notes: <%=notes%>" >
            ...
            | M </a>
            <% } %>
            <%--img src="../images/master.gif" border="0" height="10"></a--%>
        		</font></td>
        <% 
        			}
        			bFirstFirstR = false;
