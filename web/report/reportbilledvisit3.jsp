...
sql = "select distinct(bd.diagnostic_code), dt.description from billingdetail bd, diagnosticcode dt where bd.status!='D' and bd.diagnostic_code = dt.diagnostic_code and bd.appointment_date>='" + sdate + "' and bd.appointment_date<='" + edate + "' order by diagnostic_code";
rs = db.GetSQL(sql); 
while (rs.next()) { 
...
for (int i = 0; i < vServiceCode.size(); i++) {
	// get total pat
	sql = "select count(distinct(b.demographic_no)) from billing b, billingdetail bd where b.billing_no=bd.billing_no  and b.billing_date>='" + sdate + "' and b.billing_date<='" + edate + "' and b.status!='D' and bd.status!='D' and bd.diagnostic_code='" + vServiceCode.get(i) + "'";
	rs = db.GetSQL(sql); 
	while (rs.next()) { 
		props.setProperty(vServiceCode.get(i) + "pat" + vServiceDesc.get(i), rs.getString("count(distinct(b.demographic_no))"));
	}
...
*/
	...
	// get total vis
	sql = "select count(distinct(b.billing_no)) from billing b, billingdetail bd where b.billing_no=bd.billing_no  and b.billing_date>='" + sdate + "' and b.billing_date<='" + edate + "' and b.status!='D' and bd.status!='D' and bd.diagnostic_code='" + vServiceCode.get(i) + "'";
	rs = db.GetSQL(sql); 
	while (rs.next()) { 
...
*/
	...
	// get pat Physician
	sql = "select count(distinct(b.demographic_no)) from billing b, billingdetail bd, provider p where b.billing_no=bd.billing_no and b.creator=p.provider_no and b.billing_date>='" + sdate + "' and b.billing_date<='" + edate + "' and b.status!='D' and bd.status!='D' and bd.diagnostic_code='" + vServiceCode.get(i) + "' and  p.provider_type='doctor'" ;
	rs = db.GetSQL(sql); 
	while (rs.next()) { 
		props.setProperty(vServiceCode.get(i) + "patPhy" + vServiceDesc.get(i), rs.getString("count(distinct(b.demographic_no))"));
	}
...
*/
	...
	// get vis Physician
	sql = "select count(distinct(b.billing_no)) from billing b, billingdetail bd, provider p where b.billing_no=bd.billing_no and b.creator=p.provider_no and b.billing_date>='" + sdate + "' and b.billing_date<='" + edate + "' and b.status!='D' and bd.status!='D' and bd.diagnostic_code='" + vServiceCode.get(i) + "' and  p.provider_type='doctor'" ;
	rs = db.GetSQL(sql); 
	while (rs.next()) { 
		props.setProperty(vServiceCode.get(i) + "visPhy" + vServiceDesc.get(i), rs.getString("count(distinct(b.billing_no))"));
	}
...
*/
	...
	// get pat Resident
	sql = "select count(distinct(b.demographic_no)) from billing b, billingdetail bd, provider p where b.billing_no=bd.billing_no and b.creator=p.provider_no and b.billing_date>='" + sdate + "' and b.billing_date<='" + edate + "' and b.status!='D' and bd.status!='D' and bd.diagnostic_code='" + vServiceCode.get(i) + "' and  p.provider_type='resident'" ;
	rs = db.GetSQL(sql); 
	while (rs.next()) { 
		props.setProperty(vServiceCode.get(i) + "patRes" + vServiceDesc.get(i), rs.getString("count(distinct(b.demographic_no))"));
	}
...
*/
	...
	// get vis Resident
	sql = "select count(distinct(b.billing_no)) from billing b, billingdetail bd, provider p where b.billing_no=bd.billing_no and b.creator=p.provider_no and b.billing_date>='" + sdate + "' and b.billing_date<='" + edate + "' and b.status!='D' and bd.status!='D' and bd.diagnostic_code='" + vServiceCode.get(i) + "' and  p.provider_type='resident'" ;
	rs = db.GetSQL(sql); 
	while (rs.next()) { 
		props.setProperty(vServiceCode.get(i) + "visRes" + vServiceDesc.get(i), rs.getString("count(distinct(b.billing_no))"));
	}
...
*/
	...
	for (int j = 0; j < vNurseNo.size(); j++) {
		...
		rs = db.GetSQL(sql); 
		while (rs.next()) { 
			props.setProperty(vServiceCode.get(i) + "patNurse" + j + vServiceDesc.get(i), rs.getString("count(distinct(b.demographic_no))"));
		}
...
*/
		...
		sql = "select count(distinct(b.billing_no)) from billing b, billingdetail bd  where b.billing_no=bd.billing_no and bd.diagnostic_code='" + vServiceCode.get(i) + "' and b.creator='" + vNurseNo.get(j) + "' and b.billing_date>='" + sdate + "' and b.billing_date<='" + edate + "' and b.status!='D' and bd.status!='D'"  ;
		rs = db.GetSQL(sql); 
		while (rs.next()) { 
			props.setProperty(vServiceCode.get(i) + "visNurse" + j + vServiceDesc.get(i), rs.getString("count(distinct(b.billing_no))"));
		}
/*
		out.println("<hr>");
