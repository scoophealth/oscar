...
while (enumVar.hasMoreElements()) {
    ...
    if(propARSelect.containsKey(name)) { 
        ...
        if(!name.equals("ga") && !name.equals("b_primiparous"))
	        sARSelect += (sARSelect.length()<1?"":",") + "formBCAR2007."+name; 
    }
...
for(int i=0; i<vecSeqARSelect.size(); i++) {
    if(propTempARSelect.getProperty((String)vecSeqARSelect.get(i)) != null) {
        sARSelect += (sARSelect.length()<1?"":",") + "formBCAR2007." + vecSeqARSelect.get(i); 
    }
}
...
for (int i = 0; i < vecValue.size(); i++) {
	...
	if(strFilter.indexOf("formBCAR2007.")>=0) {
	    bARFilter = true;
		...
		if(strFilter.indexOf("formBCBirthSumMo") > 0) {
			...
			strFilter = "formBCAR2007.demographic_no in (" + sBirthSumNo + ")";
		}
...
if( (bDemoSelect && bARSelect && !bSpecSelect && !bSpecFilter) || (!bSpecSelect && bARFilter && !bSpecFilter) ) {
    ...
    String subQuery = "select max(ID) from formBCAR2007, demographic where demographic.demographic_no=formBCAR2007.demographic_no ";
    
    ///here is the prob
    
    subQuery += " and " + sDemoFilter + sTempEle + " group by formBCAR2007.demographic_no,formBCAR2007.formCreated ";
	System.out.println(" demographic and formBCAR2007 subQuery: " + subQuery);
    String subFormId = "";
    ...
    String sql = "select demographic.demographic_no," + sDemoSelect + sTempEle + " from demographic,formBCAR2007 where ";
    sql += " formBCAR2007.ID in (" + subFormId + ") and demographic.demographic_no=formBCAR2007.demographic_no " + ORDER_BY;
	System.out.println(" demographic and formBCAR2007: " + sql);
	...
	if(bARSelect) {
		temp = sARSelect.replaceAll("formBCAR2007.","").split(",");
		for(int i=0; i<temp.length; i++) {
...
if( (bDemoSelect && bARSelect && bSpecSelect) || (bARFilter && bSpecFilter) ) {
    if(bDemoSelect && bARSelect && bSpecSelect && !bSpecFilter) {
        ...
        String subQuery = "select max(ID) from formBCAR2007, demographic where demographic.demographic_no=formBCAR2007.demographic_no ";
        subQuery += " and " + sDemoFilter + sTempEle + " group by formBCAR2007.demographic_no,formBCAR2007.formCreated ";
    	System.out.println(" demographic and formBCAR 2007= subQuery: " + subQuery);
        String subFormId = "";
        ResultSet rs = dbObj.searchDBRecord(subQuery);
    	while (rs.next()) {
    	    subFormId += (subFormId.length()>0? ",":"") +rs.getInt("max(ID)");
	...
	if(bARFilter && bSpecFilter) {
        ...
        subQuery = "select max(ID) from formBCAR2007, demographic where demographic.demographic_no=formBCAR2007.demographic_no ";
        subQuery += " and " + sDemoFilter + sTempEle + " group by formBCAR2007.demographic_no,formBCAR2007.formCreated ";
    	System.out.println(" demographic and formBCAR2007 subQuery: " + subQuery);
        String subFormId = "";
        rs = dbObj.searchDBRecord(subQuery);
    	...
    	sTempEle = sARSelect.length()>0? (","+sARSelect) : "";
    	subFormId = subFormId.length()>0? subFormId : "0";
        sql = "select demographic.demographic_no," + sDemoSelect + sTempEle + " from demographic,formBCAR2007 where ";
