...
<nested:form action="/CaseManagementEntry"
	...
	%>
		<div id="nc<%=idx%>" class="note"><input type="hidden" id="signed<%=note.getNoteId()%>" value="<%=note.isSigned()%>"> <input type="hidden" id="full<%=note.getNoteId()%>" value="<%=fulltxt || (note.getNoteId() !=null && note.getNoteId().equals(savedId))%>"> <input type="hidden"
