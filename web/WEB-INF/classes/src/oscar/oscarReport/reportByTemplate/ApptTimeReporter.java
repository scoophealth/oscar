 ...
 * @author rjonasz
 */
public class ApptTimeReporter implements Reporter{

    private class Provider {
        public String name = "";
        public Integer providerMins = 0;
    }
    
    /**
     * Creates a new instance of UnusedMinutesReporter
     */
    public ApptTimeReporter() {
    }
    ...
    public boolean generateReport( HttpServletRequest request ) {
        ...
        String rsHtml = "";
        String csv = "";
        if( date_from == null ||  date_to == null || providers == null || schedSymbols == null ) {
            rsHtml = "dates and provider must be set and at least one schedule symbol must be set";
        ...
        String scheduleSQL = "select scheduledate.provider_no, scheduletemplate.timecode, scheduledate.sdate from scheduletemplate, scheduledate where scheduletemplate.name=scheduledate.hour and scheduledate.sdate >= '" + date_from +  "' and scheduledate.sdate <= '" + date_to + "' and  scheduledate.provider_no in (" + providerList.toString() + ") and scheduledate.status = 'A' and (scheduletemplate.provider_no=scheduledate.provider_no or scheduletemplate.provider_no='Public') order by scheduledate.sdate";
        ...
        int unbooked = 0;
        int totalMins = 0;
        HashMap<String,Provider>providerTotals = new HashMap<String,Provider>();
        ...
        try {
            ...
            while(rs.next()) {
                ...
                tmpApptSQL = "select a.start_time, a.end_time from appointment a where " +
                        "a.provider_no = '" + rs.getString("provider_no") + "' and a.status not like '%C%' and " +
                        "a.appointment_date = '" + schedDate + "' order by start_time asc";
                ...
                unbooked = 0;
                provider = providerTotals.get(rs.getString("provider_no"));
                if( provider == null ) {
                    provider = new Provider();
                    provNameSQL = "select concat(last_name,', ', first_name) name from provider where provider_no = '" + rs.getString("provider_no") + "'";
                    rs3 = DBHandler.GetSQL(provNameSQL);
                    if( rs3.next() ) {
                        provider.name = rs3.getString("name");
                    }
                    rs3.close();
                }
                    ...
                    for( int schedIdx = 0; schedIdx < schedSymbols.length; ++schedIdx ) {
                        ...
                        if( code.equals(schedSymbols[schedIdx]) ) {                        
                            ...
                            if( iHours > latestApptHour || (iHours == latestApptHour && iMins > latestApptMin)) {
                                totalMins += duration;
                                provider.providerMins += duration;
                                ++numAppts;
//                              
                            }
...
//                    }
                ...
                providerTotals.put(rs.getString("provider_no"), provider);
            } //end while
...
//            }
        ...
        rsHtml = makeHTML(providerTotals,totalMins);
        csv = makeCSV(providerTotals,totalMins);
    ...
    private String makeHTML(HashMap<String,Provider>providerTotals, int totalMins) {
        String html = "<table class=\"reportTable\">\n";                        
        if( totalMins == 0 ) {
            html +=  "<tr class=\"reportRow1\"><td>There is no time available" + "</td>\n</tr>\n</table>\n";
        }
        else {
            html += "<tr><th class=\"reportHeader\">Provider</th><th class=\"reportHeader\">Total Mins</th></tr>";

            Provider p;
            Set<String> keys = providerTotals.keySet();
            Integer hours, mins;
            for( String key : keys ) {
                p = providerTotals.get(key);
                hours = p.providerMins/60;
                mins = p.providerMins%60;
                html += "<tr class=\"reportRow1\"><td>" + p.name + "</td>\n<td>" + hours + " hrs " + mins +  " mins</td></tr>";
            }

            hours = totalMins/60;
            mins = totalMins%60;
            html += "<tr class=\"reportRow1\"><td>Total</td><td>" + hours + " hrs " + mins + " mins</td></tr>";
            html += "</table>";
        }
    ...
    private String makeCSV(HashMap<String,Provider>providerTotals, int totalMins) {
        String csv = "";
        Provider p;
        Integer hours, mins;
        Set<String> keys = providerTotals.keySet();
        for( String key : keys ) {
            ...
            hours = p.providerMins/60;
            mins = p.providerMins%60;
            csv = "Provider," + p.name + ",Total Mins," + hours + " hrs " + mins +  " mins\n";
        }
        hours = totalMins/60;
        mins = totalMins%60;

        csv += "Total," + hours + " hrs " + mins + " mins,,\n";
        return csv;
    }
