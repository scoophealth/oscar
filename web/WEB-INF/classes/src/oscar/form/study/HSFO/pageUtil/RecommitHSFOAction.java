...
public class RecommitHSFOAction extends DispatchActionSupport
{
			...
			throws Exception
	{
		...
		{
			request.setAttribute("schedule_err",
					"Invalid time,please schedule another time!");
				...
				throws JobExecutionException
		{
			try
			{
				...
				if (!"D".equalsIgnoreCase(rs.getStatus()))
				{
					...
					HsfoHbpsDataDocument doc = tfutil.generateXML(rs
							.getUser_no(), 0);
					...
					String rstr = "<?xml version=\"1.0\" encoding=\"utf-8\"?>"
							+ doc.xmlText();
					// send to hsfo web

					try
					{
						message = tfutil.soapHttpCall(tfutil.getSiteID()
								.intValue(), tfutil.getUserId(), tfutil
								.getLoginPasswd(), rstr);
					} catch (Exception e)
					{
						logger.error(e);
