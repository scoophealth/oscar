...
public class RxPrescriptionData {
	...
	public Favorite[] getFavorites(String providerNo) {
		...
		try {
			...
			while (rs.next()) {
				favorite = new Favorite(rs.getInt("favoriteid"), db.getString(rs, "provider_no"), db.getString(rs, "favoritename"), db.getString(rs, "BN"), rs.getInt("GCN_SEQNO"), db.getString(rs, "customName"), rs.getFloat("takemin"), rs
				        .getFloat("takemax"), db.getString(rs, "freqcode"), db.getString(rs, "duration"), db.getString(rs, "durunit"), db.getString(rs, "quantity"), rs.getInt("repeat"), rs.getInt("nosubs"), rs.getInt("prn"), db.getString(rs, "special"),
	...
	public String saveScript(oscar.oscarRx.pageUtil.RxSessionBean bean) {
		...
		String sql = " insert into prescription " + " (provider_no,demographic_no,date_prescribed,date_printed,textView) " + " values " + " ( '" + provider_no + "', " + "   '" + demographic_no + "', " + "   '" + date_prescribed + "', " + "   '"
		        + date_printed + "', " + "   '" + StringEscapeUtils.escapeSql(textView.toString()) + "') ";
		try {
	...
	public class Prescription {
		...
		public boolean registerIndivo() {
			...
			try {
				String update;
				if (isRegisteredIndivo()) update = "U";
				else update = "I";
		...
		public boolean Print() {
			...
			try {
				...
				if (rs.next()) {
					...
					if (dates_reprinted != null && dates_reprinted.length() > 0) dates_reprinted += "," + now;
					else dates_reprinted = now;
		...
		public boolean Save(String scriptId) {
			...
			try {
				...
				if (this.getDrugId() == 0) {
					...
					sql = "SELECT drugid FROM drugs WHERE archived = 0 AND " + "provider_no = '" + this.getProviderNo() + "' AND " + "demographic_no = " + this.getDemographicNo() + " AND " + "rx_date = '" + RxUtil.DateToString(this.getRxDate()) + "' AND "
					        + "end_date = '" + RxUtil.DateToString(this.getEndDate()) + "' AND " + "written_date = '" + RxUtil.DateToString(this.getWrittenDate()) + "' AND " + "BN = '" + this.getBrandName() + "' AND " + "GCN_SEQNO = "
					...
					if (this.getDrugId() == 0) {
						sql = "INSERT INTO drugs (provider_no, demographic_no, " + "rx_date, end_date, written_date, BN, GCN_SEQNO, customName, " + "takemin, takemax, freqcode, duration, durunit, quantity, "
						        ...
						        + "', '"
						        + StringEscapeUtils.escapeSql(this.getBrandName())
						        + "', "
						        + this.getGCN_SEQNO()
						        + ", '"
						        ...
						        + this.getRepeat()
						        + ", '"
						        + RxUtil.DateToString(this.getLastRefillDate())
						        + "', "
						        + this.getNosubsInt()
						        + ", "
						        + this.getPrnInt()
					...
					sql = "UPDATE drugs SET " + "provider_no = '" + this.getProviderNo() + "', " + "demographic_no = " + this.getDemographicNo() + ", " + "rx_date = '" + RxUtil.DateToString(this.getRxDate()) + "', " + "end_date = '"
					        ...
					        + this.getPatientCompliance() + " " + "WHERE drugid = " + this.getDrugId();
		...
		public boolean AddToFavorites(String providerNo, String favoriteName) {
			...
			Favorite fav = new Favorite(0, providerNo, favoriteName, this.getBrandName(), this.getGCN_SEQNO(), this.getCustomName(), this.getTakeMin(), this.getTakeMax(), this.getFrequencyCode(), this.getDuration(), this.getDurationUnit(), this
			        .getQuantity(), this.getRepeat(), this.getNosubsInt(), this.getPrnInt(), this.getSpecial(), this.getGenericName(), this.getAtcCode(), this.getRegionalIdentifier(), this.getUnit(), this.getUnitName(), this.getMethod(), this.getRoute(),
