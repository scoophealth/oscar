...
public class DBQuery {
  ...
  public boolean isNewPatientOrNot(String name){
     String nameString ="-";
     String getQuery = " select last_name,first_name from demographic " ;
         ResultSet RS = null;
                  try{
                    DBConnect dbc= new DBConnect();
                    RS = dbc.executeQuery(getQuery);
                       while (RS.next()){
                           nameString = nameString + RS.getString("last_name").trim()+","+RS.getString("first_name").trim()+"-";	
                       }
                    } catch(Exception ex) {
                    System.err.println(" select * from appointment : " + ex.getMessage());
                    }
