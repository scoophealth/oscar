...
public class ConsentManagerImpl implements ConsentManager {
    ...
    public void saveConsent(Consent consent) {
        ...
        if ("consent not given".equals(consent.getStatus())) {
            clientDao.saveDemographicExt(consent.getDemographicNo().intValue(), Demographic.CONSENT_GIVEN_KEY ,Demographic.ConsentGiven.NONE.name());
        }
        if ("consent given".equals(consent.getStatus())) {
            clientDao.saveDemographicExt(consent.getDemographicNo().intValue(), Demographic.CONSENT_GIVEN_KEY ,Demographic.ConsentGiven.CIRCLE_OF_CARE.name());
        }
