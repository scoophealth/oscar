...
public class FlowSheetDrugDAO extends HibernateDaoSupport {
    ...
    public List<FlowSheetDrug> getFlowSheetDrugs(String flowsheet,String demographic){
        List<FlowSheetDrug> list = this.getHibernateTemplate().find("from FlowSheetDrug p where p.flowsheet = ? and p.archived = 0 and p.demographicNo = ?   ", new Object[] {flowsheet,Integer.parseInt(demographic)});
        return list;
    }
