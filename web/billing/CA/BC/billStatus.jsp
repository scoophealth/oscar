...
<jsp:useBean id="apptMainBean" class="oscar.AppointmentMainBean"
	scope="session" /><jsp:useBean id="SxmlMisc" class="oscar.SxmlMisc"
	scope="session" /><%@ include file="../../../admin/dbconnection.jsp"%>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
	<tr bgcolor="#000000">
		...
		<p><font color="#FFFFFF" size="4"
			face="Arial, Helvetica, sans-serif"><b>oscar<font size="3">Billing
		- Invoice List</font></b></font></p>
		</td>
...
%>
<div><i>Results for Demographic</i> : <%=request.getParameter("lastName")%>
, <%=request.getParameter("firstName")%> ( <%=request.getParameter("demographicNo")%>
)</div>
<%}%>
<table width="100%" border="0" bgcolor="#EEEEFF">
	<form name="serviceform" method="get" action="billStatus.jsp"><input
		type="hidden" name="filterPatient" value="<%=readonly%>" /> <input
		type="hidden" name="lastName"
		value="<%=request.getParameter("lastName")%>" /> <input type="hidden"
		name="firstName" value="<%=request.getParameter("firstName")%>" /> <!--<input type="hidden" name="demographicNo" value="<%=demographicNo%>"/>-->
	<tr>
		<td rowspan="2">
		<table>
			<tr>
				<td class="bCellData"><input type="checkbox" name="showMSP"
					value="show" <%=showMSP?"checked":""%> /> <a
					onclick="billTypeOnly('showMSP')">MSP</a></td>
			</tr>
			<tr>
				<td class="bCellData"><input type="checkbox" name="showWCB"
					value="show" <%=showWCB?"checked":""%> /> <a
					onclick="billTypeOnly('showWCB')">WCB</a></td>
			</tr>
			<tr>
				<td class="bCellData"><input type="checkbox" name="showPRIV"
					value="show" <%=showPRIV?"checked":""%> /> <a
					onClick="billTypeOnly('showPRIV')">Private</a></td>
			</tr>
			<tr>
				<td class="bCellData"><input type="checkbox" name="showICBC"
					value="show" <%=showICBC?"checked":""%> /> <a
					onClick="billTypeOnly('showICBC')">ICBC</a></td>
			</tr>
		</table>
		</td>
		<td colspan=2>
		...
		provider </b></font> <select name="providerview">
			...
			<%=proFirst%></option>
			<%  } %>
		</select> <font color="#333333" size="2"
			face="Verdana, Arial, Helvetica, sans-serif"> <input
			type="hidden" name="verCode" value="V03" /> <input type="submit"
			name="Submit" value="Create Report"> </font></div>
		</td>
	</tr>
	<tr>
		<td class="bCellData">
		<div align="center"><font color="#333333">Service
		Date-Range</font> &nbsp;&nbsp; <font size="1"
			face="Arial, Helvetica, sans-serif"> <a
			href="javascript: function myFunction() {return false; }"
			id="hlSDate">Begin:</a> </font> <input type="text" name="xml_vdate"
			id="xml_vdate" value="<%=xml_vdate%>"> <font size="1"
			face="Arial, Helvetica, sans-serif"> <a
			href="javascript: function myFunction() {return false; }"
			id="hlADate">End:</a> </font> <input type="text"
			name="xml_appointment_date" id="xml_appointment_date"
			value="<%=xml_appointment_date%>"> <a
			href="javascript: function myFunction() {return false; }"
		...
		%> <input type="text" name="demographicNo" size="5"
			value="<%=xml_demoNo%>" <%=readonlyStr%>"/></div>
		</td>
		<td class="bCellData">
		<div align="right"><input type='button' name='print'
			value='Print' onClick='window.print()'></div>
		</td>
	</tr>
</table>
...
<table>
	<tr>
		<td><input type="radio" name="billTypes"
			value="<%=MSPReconcile.REJECTED%>"
			<%=billTypes.equals(MSPReconcile.REJECTED)?"checked":""%> /> Rejected
		<input type="radio" name="billTypes"
			value="<%=MSPReconcile.NOTSUBMITTED%>"
			<%=billTypes.equals(MSPReconcile.NOTSUBMITTED)?"checked":""%> /> Not
		Submitted <input type="radio" name="billTypes"
			value="<%=MSPReconcile.SUBMITTED%>"
			<%=billTypes.equals(MSPReconcile.SUBMITTED)?"checked":""%> />
		Submitted <input type="radio" name="billTypes"
			value="<%=MSPReconcile.SETTLED%>"
			<%=billTypes.equals(MSPReconcile.SETTLED)?"checked":""%> /> Settled <input
			type="radio" name="billTypes" value="<%=MSPReconcile.DELETED%>"
			<%=billTypes.equals(MSPReconcile.DELETED)?"checked":""%> /> Deleted <input
			type="radio" name="billTypes" value="<%=MSPReconcile.HELD%>"
			<%=billTypes.equals(MSPReconcile.HELD)?"checked":""%> /> Held <input
			type="radio" name="billTypes"
			value="<%=MSPReconcile.DATACENTERCHANGED%>"
			title="Data Center Changed"
			<%=billTypes.equals(MSPReconcile.DATACENTERCHANGED)?"checked":""%> />
		DCC <input type="radio" name="billTypes"
			value="<%=MSPReconcile.PAIDWITHEXP%>" title="Paid with Explanation"
			<%=billTypes.equals(MSPReconcile.PAIDWITHEXP)?"checked":""%> /> PwE <input
			type="radio" name="billTypes" value="<%=MSPReconcile.BADDEBT%>"
			<%=billTypes.equals(MSPReconcile.BADDEBT)?"checked":""%> /> Bad Debt
		<input type="radio" name="billTypes" value="<%=MSPReconcile.REFUSED%>"
			<%=billTypes.equals(MSPReconcile.REFUSED)?"checked":""%> /> Refused <!--<input type="radio" name="billTypes" value="<%=MSPReconcile.WCB%>"          <%=billTypes.equals(MSPReconcile.WCB)?"checked":""%>/> WCB-->
		<input type="radio" name="billTypes"
			value="<%=MSPReconcile.CAPITATED%>" title="Capitated"
			<%=billTypes.equals(MSPReconcile.CAPITATED)?"checked":""%> /> Cap <input
			type="radio" name="billTypes" value="<%=MSPReconcile.DONOTBILL%>"
			title="Do Not Bill"
			<%=billTypes.equals(MSPReconcile.DONOTBILL)?"checked":""%> /> DNBill
		<input type="radio" name="billTypes"
			value="<%=MSPReconcile.BILLPATIENT%>"
			<%=billTypes.equals(MSPReconcile.BILLPATIENT)?"checked":""%> /> Bill
		Patient <input type="radio" name="billTypes"
			value="<%=MSPReconcile.PAIDPRIVATE%>" title="Paid Private"
			<%=billTypes.equals(MSPReconcile.PAIDPRIVATE)?"checked":""%> />
		PPrivate <input type="radio" name="billTypes"
			value="<%=MSPReconcile.COLLECTION%>" title="Transfered to Collection"
			<%=billTypes.equals(MSPReconcile.COLLECTION)?"checked":""%> />
		Collection <input type="radio" name="billTypes" value="%"
			<%=billTypes.equals("%")?"checked":""%> /> All <input type="radio"
			name="billTypes" value="?" <%=billTypes.equals("?")?"checked":""%> />
		Fixable Receivables <input type="radio" name="billTypes" value="$"
			<%=billTypes.equals("$")?"checked":""%> /> Paid Bills <input
			type="hidden" name="submitted" value="yes" /></td>
	</tr>
</table>
</form>
<table class="sortable tabular_list" id="table-1" width="100%"
	border="2" valign="top">
	<thead>
		<tr bgcolor="#CCCCFF">
			<th align="center" class="bHeaderData" title="INVOICE #">INVOICE
			#</th>
			<th align="center" class="bHeaderData" title="LINE #">SEQ #</th>
			<th align="center" class="bHeaderData" title="APP. DATE">APP.
			DATE</th>
   ...
   %>
		...
		<tr bgcolor="<%=bodd?"#EEEEFF":"white"%>">
			...
			<%if("Pri".equals(b.billingtype)){%> <a
				href="javascript:popupPage(800,800, '../../../billing/CA/BC/billingView.do?billing_no=<%=b.billing_no%>&receipt=yes')"><%=b.billing_no%>
			</a> <%}
	 else{
		%> <%=b.billing_no%> <%}%>
			</td>
			...
			<td align="center" class="bCellData"><a
				href="javascript: function myFunction() {return false; }"
				onClick="popupPage2(500,1020,'genTAS00ByOfficeNo.jsp?officeNo=<%=b.billMasterNo%>','RecValues');">
			<%=b.seqNum%> </a></td>
	...
	%>
			<td align="center" class="bCellData"><a
				href="javascript: setDemographic('<%=b.demoNo%>');"><%=b.demoName%></a></td>
			<%}%>
			<td align="center" class="bCellData"><%=b.providerLastName%>,<%=b.providerFirstName%></td>
			<td align="center" class="bCellData"
				title="<%=msp.getStatusDesc(b.reason)%>"><%=msp.getStatusDesc(b.reason)==null?"&nbsp":msp.getStatusDesc(b.reason)%></td>


			<td align="center" class="bCellData"><%=b.code%></td>
			<td align="center" class="bCellData" <%=isBadVal(incorrectVal)%>><%=b.quantity%></td>
			<td align="center" class="bCellData" <%=isBadVal(incorrectVal)%>><%=nf.format(Double.parseDouble(b.amount))%>
			</td>
    ...
    %>
			...
			<td>
			<% if (adminAccess){ %> <%if (!b.isWCB()){%> <a
				href="javascript: popupPage(700,700,'adjustBill.jsp?billing_no=<%=b.billMasterNo%>&invoiceNo=<%=b.billing_no%>')">Edit
			</a> <%}else{%> <a
				href="javascript: popupPage(700,700,'billingTeleplanCorrectionWCB.jsp?billing_no=<%=b.billMasterNo%>')">Edit
			</a> <%}%> <% } %> <%=rejected%><%=rejected2%></td>
		</tr>
...
<script language="javascript"
	src="../../../commons/scripts/sort_table/css.js">
