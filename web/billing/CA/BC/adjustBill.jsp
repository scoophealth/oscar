...
<table width="100%" border="0" bgcolor="#FFFFFF">
  <tr>
    <td width="20%" align="left" class="bCellData">
      <font color="#000000">Office Claim No
    </td>
    <td width="20%" class="bCellData"><%=billNo%>    </td>
    <td width="60%" align="left" class="bCellData">
      <font color="#000000">        Last update:
<%=UpdateDate%>      </font>
    </td>
  </tr>
</table>
...
%>
<br>
<html:form action="/billing/CA/BC/reprocessBill" onsubmit="checkSubmitType()">
  ...
  <table width="100%" border="0">
    <tr bgcolor="#CCCCFF">
      <td height="21" colspan="2" class="bCellData">        Patient Information
        <input type="hidden" name="billingmasterNo" value="<%=billNo%>"/>
    ...
    <tr>
      <td width="54%" class="bCellData">        Patient Name:
        <a href=# onClick="popupPage2('../../../demographic/demographiccontrol.jsp?demographic_no=<%=DemoNo%>&displaymode=edit&dboperation=search_detail');return false;" title="<bean:message key="provider.appointmentProviderAdminDay.msgMasterFile"/>"><%=DemoName%>        </a>
        <input type="hidden" name="demo_name" value="<%=DemoName%>">
      </td>
      <td width="46%" class="bCellData">        Health# :
      <%if (HCTYPE != null && HCTYPE.equals("BC")) {      %>
<%=allFields.getProperty("phn")%>      <%} else {      %>
<%=allFields.getProperty("oin_registration_no")%>      <%}      %>
        Type
<%=HCTYPE%>      </td>
    </tr>
    <tr bgcolor="#EEEEFF">
      <td class="bCellData">        Sex:
<%=DemoSex%>        <input type="hidden" name="demo_sex" value="<%=DemoSex%>">
        <input type="hidden" name="hc_sex" value="<%=HCSex%>">
      </td>
      <td class="bCellData">        D.O.B. :
<%=DemoDOB%>        <input type="hidden" name="xml_dob" value="<%=DemoDOB%>">
      </td>
    </tr>
    <tr>
      <td class="bCellData">        Address:
<%=DemoAddress%>        <input type="hidden" name="demo_address" value="<%=DemoAddress%>">
      </td>
      <td class="bCellData">        City:
<%=DemoCity%>        <input type="hidden" name="demo_city" value="<%=DemoCity%>">
      </td>
    </tr>
    <tr bgcolor="#EEEEFF">
      <td class="bCellData">        Province:
<%=DemoProvince%>        <input type="hidden" name="demo_province" value="<%=DemoProvince%>">
      </td>
      <td class="bCellData">        Postal Code:
<%=DemoPostal%>        <input type="hidden" name="demo_postal" value="<%=DemoPostal%>">
      </td>
    </tr>
  </table>
  <table width="100%" border="0">
    <tr bgcolor="#CCCCFF">
      <td colspan="2" class="bCellData">        Billing Information  Data Center
<%=allFields.getProperty("datacenter")%>        Payee Number:
<%=allFields.getProperty("payee_no")%>        Practitioner Number:
<%=allFields.getProperty("practitioner_no")%>      </td>
    </tr>
    <tr>
      <td class="bCellData">
...
<!--<a href="#" onClick='rs("billingcalendar","<rewrite:reWrite jspPage="billingCalendarPopup.jsp"/>?year=<%=curYear%>&month=<%=curMonth%>&type=&returnForm=ReProcessBilling&returnItem=serviceDate","380","300","0")'>-->
              <a href="javascript: function myFunction() {return false; }" id="hlSDate">Billing Date:              </a>
            </td>
            <td>To:            </td>
          </tr>
          <tr>
            <td>
              <input type="text" style="font-size:80%;" id="serviceDate" name="serviceDate" value="<%=allFields.getProperty("service_date")%>">
<!--<%=allFields.getProperty("service_date")%>"/><%=BillDate%>-->
            </td>
            <td>
    ...
    <%visittype = allFields.getProperty("service_location");    %>
      <td width="54%" class="bCellData">        Visit Type:
      ...
      <td width="46%" class="bCellData">
        ...
        <a href="#" onClick='rs("billingcalendar","billingCalendarPopup.jsp?year=<%=curYear%>&month=<%=curMonth%>&type=&returnForm=serviceform&returnItem=xml_vdate","380","300","0")'>Admission Date:        </a>
    ...
    <tr>
      <td class="bCellData">        After Hours:
        <select name="afterHours">
          ...
          <option value="E">Evening Call</option>
          <option value="N">Night Call</option>
          <option value="W">Weekend Call</option>
        </select>
      </td>
      <td class="bCellData">        Time Call Recieved
        <!--TIME-CALL-RECVD-SRV-->
    ...
    <tr>
      <td class="bCellData">        Service Time Start
      <%!/*SERVICE-TIME-START*/      %>
        <input type="text" name="startTime" value="<%=allFields.getProperty("service_start_time")%>" size="4" maxlength="4"/>
      </td>
      <td class="bCellData">        Service Time Finish
      <%!/*SERVICE-TIME-FINISH*/      %>
        <input type="text" name="finishTime" value="<%=allFields.getProperty("service_end_time")%>" size="4" maxlength="4"/>
      </td>
    </tr>
    <tr>
...
<!--<input type="text" name="mvaClaim" value="<%=allFields.getProperty("mva_claim_code")%>" size="1"/>-->
      ...
      <td class="bCellData">        ICBC Claim Num:
        <input type="text" name="icbcClaim" value="<%=allFields.getProperty("icbc_claim_no")%>" size="8" maxlength="8"/>
      </td>
    </tr>
    <tr>
      <td class="bCellData">        Facility Number
        <input type="text" name="facilityNum" value="<%=allFields.getProperty("facility_no")%> " size="5" maxlength="5"/>
      </td>
      <td class="bCellData">        Facility Sub Number
        <input type="text" name="facilitySubNum" value="<%=allFields.getProperty("facility_sub_no")%> " size="5" maxlength="5"/>
      </td>
    </tr>
  </table>
  <table width="100%" border=1>
    ...
    <tr>
      ...
      <td width="50%" class="bCellData"><%=billform.getServiceDesc(allFields.getProperty("billing_code"),billRegion)%>      </td>
      <td class="bCellData">
  ...
  %>
    <tr>
        ...
        <table width="100%">
          ...
          <tr>
            <td class="bCellData">
              ...
              <input type="text" name="dx1" value="<%=allFields.getProperty("dx_code1")%>" size="10">
            </td>
          ...
          <tr>
            <td class="bCellData">
              ...
              <input type="text" name="dx2" value="<%=allFields.getProperty("dx_code2")%>" size="10">
            </td>
          ...
          <tr>
            <td class="bCellData">
              ...
              <input type="text" name="dx3" value="<%=allFields.getProperty("dx_code3")%>" size="10">
            </td>
      ...
      <td colspan=2 valign="top">
        <table width="100%">
          <tr bgcolor="#CCCCFF">
            <td colspan=2 class="bCellData">              Referrals
            <%
    ...
    <tr>
      ...
      <td class="bCellData">
        <select name="paymentMode">
          ...
          <option value="E">Alternate Funding</option>
        </select>
      </td>
      ...
      <td class="bCellData">
        <select name="submissionCode" onChange="checkDebitRequest();">
          <option value="0">0|Normal Submission</option>
          <option value="D">D|Duplicate</option>
          <option value="E">E|Debit Request</option>
          <option value="I">I|ICBC Claim</option>
          <option value="W">W|Claim not accepted by WCB</option>
          <option value="C">C|Subscriber Coverage Problem</option>
          <option value="R">R|Resubmit Claim</option>
          <option value="A">A|Pre-approved claim</option>
          <option value="X">X|Resubmitting refused or part paid</option>
        </select>
      </td>
    </tr>
    <tr>
      <td class="bCellData" colspan="4">
        <div id="DEBITREQUEST">          Select sequence number you would like to debit
          <input name="debitRequestSeqNum" type="text" maxlength="7" size="7" value="<%=getDebitRequestSeqNum(allFields.getProperty("original_claim"))%>"/>
</br>          Select date MSP received claim (if not known, fill with zeros) (YYYYMMDD):
          <input id="debitRequestDate" name="debitRequestDate" type="text" maxlength="8" size="8" value="<%=getDebitRequestDate(allFields.getProperty("original_claim"))%>"/>
          <a id="hlADate">
    ...
    <tr>
      ...
      <td class="bCellData">
        <select name="correspondenceCode" onChange="correspondenceNote();">
          <option value="0">None</option>
          <option value="C">Paper Note</option>
          <option value="N">Elec Note</option>
...
<!--<input type="text" name="correspondenceCode" value="<%=allFields.getProperty("correspondence_code")%>" size="1"/>-->
      ...
      <td class="bCellData">
        <select name="insurerCode2">
          ...
          <option value="IN">Institutional Claim</option>
          <option value="PP">Pay Patient</option>
          <option value="WC">WCB</option>
        </select>
      </td>
    </tr>
    <tr>
      ...
      <!--CLAIM-SHORT-COMMENT-->
      <td>
        <input type="text" name="shortComment" value="<%=allFields.getProperty("claim_comment")%>" size="20" maxlength="20"/>
      </td>
