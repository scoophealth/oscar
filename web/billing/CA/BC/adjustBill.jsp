...
<%@page
	import="java.math.*, java.util.*, java.sql.*, oscar.*, java.net.*,oscar.oscarBilling.ca.bc.MSP.*"%>
...
<table width="100%" border="0" bgcolor="#FFFFFF">
	<tr>
		<td width="20%" align="left" class="bCellData"><font
			color="#000000"> Office Claim No </td>
		<td width="20%" class="bCellData"><%=billNo%></td>
		<td width="60%" align="left" class="bCellData"><font
			color="#000000">Last update: <%=UpdateDate%></font></td>
	</tr>
...
%>
<br>
<html:form action="/billing/CA/BC/reprocessBill"
	onsubmit="checkSubmitType()">
	...
	<input type="hidden" name="billNumber"
		value="<%=allFields.getProperty("billing_no")%>" />
	<table width="100%" border="0">
		<tr bgcolor="#CCCCFF">
			<td height="21" colspan="2" class="bCellData">Patient
			Information<input type="hidden" name="billingmasterNo"
				value="<%=billNo%>" /> <%
...
%>
		...
		<tr>
			<td width="54%" class="bCellData">Patient Name: <a href=#
			...
			<%=DemoName%> </a> <input type="hidden" name="demo_name"
				value="<%=DemoName%>"></td>
			<td width="46%" class="bCellData">Health# : <% if (HCTYPE != null && HCTYPE.equals("BC")){ %>
			<%=allFields.getProperty("phn")%> <%}else{%> <%=allFields.getProperty("oin_registration_no")%>
			<%}%> Type <%=HCTYPE%></td>
		</tr>
		<tr bgcolor="#EEEEFF">
			<td class="bCellData">Sex: <%=DemoSex%> <input type="hidden"
				name="demo_sex" value="<%=DemoSex%>"> <input type="hidden"
				name="hc_sex" value="<%=HCSex%>"></td>
			<td class="bCellData">D.O.B. : <%=DemoDOB%> <input type="hidden"
				name="xml_dob" value="<%=DemoDOB%>"></td>
		</tr>
		<tr>
			<td class="bCellData">Address: <%=DemoAddress%> <input
				type="hidden" name="demo_address" value="<%=DemoAddress%>">
			</td>
			<td class="bCellData">City: <%=DemoCity%> <input type="hidden"
				name="demo_city" value="<%=DemoCity%>"></td>
		</tr>
		<tr bgcolor="#EEEEFF">
			<td class="bCellData">Province: <%=DemoProvince%> <input
				type="hidden" name="demo_province" value="<%=DemoProvince%>">
			</td>
			<td class="bCellData">Postal Code: <%=DemoPostal%> <input
				type="hidden" name="demo_postal" value="<%=DemoPostal%>"></td>
		</tr>
	</table>
	...
	<table width="100%" border="0">
		<tr bgcolor="#CCCCFF">
			<td colspan="2" class="bCellData">Billing Information Data
			Center <%=allFields.getProperty("datacenter")%> Payee Number: <%=allFields.getProperty("payee_no")%>
			Practitioner Number: <%=allFields.getProperty("practitioner_no")%></td>
		</tr>

		<tr>
			<td class="bCellData"><!-- includes the Billing Type Drop Down List -->
			...
			<table>
				<tr>
					<td><!--<a href="#" onClick='rs("billingcalendar","<rewrite:reWrite jspPage="billingCalendarPopup.jsp"/>?year=<%=curYear%>&month=<%=curMonth%>&type=&returnForm=ReProcessBilling&returnItem=serviceDate","380","300","0")'>-->
					<a href="javascript: function myFunction() {return false; }"
						id="hlSDate"> Billing Date: </a></td>
					<td>To:</td>
				</tr>
				<tr>
					<td><input type="text" style="font-size: 80%;"
						id="serviceDate" name="serviceDate"
						value="<%=allFields.getProperty("service_date")%>"><!--<%=allFields.getProperty("service_date")%>"/><%=BillDate%>-->
					</td>
					<td><input type="text" name="serviceToDay"
						value="<%=allFields.getProperty("service_to_day")%>" size="2"
						maxlength="2" /></td>
				</tr>
			</table>
			</td>
		...
		<tr bgcolor="#EEEEFF">
			<td width="54%" class="bCellData">Clarification Code: <input
				type="text" name="locationVisit"
				value="<%=allFields.getProperty("clarification_code")%>"
				maxlength="2" size="2" /></td>
			<td width="46%" class="bCellData">Billing Physician#: <select
				style="font-size: 80%;" name="providerNo">
		...
		<tr>
			...
			<td width="54%" class="bCellData">Visit Type: <input
				type="hidden" name="xml_visittype" value="<%=visittype%>"> <select
				name="serviceLocation" style="font-size: 80%;">
				<%
              ...
              %>
				<option value="<%=visit.getVisitType()%>" <%=selected%>><%=visit.getDescription()%>
				</option>
				<%
              }
              %>
			...
			<td width="46%" class="bCellData"><input type="hidden"
				name="xml_visitdate" value="<%=visitdate%>"> <a href="#"
			...
			Admission Date: </a> <input type="text" style="font-size: 80%;"
				name="xml_vdate" value="<%=visitdate%>"></td>
		</tr>
		<tr>
			<td class="bCellData">Dependent Number: <select
				name="dependentNo">
				<option value="00"
					<%=allFields.getProperty("dependent_num").equals("00")?"selected":""%>>00</option>
				<option value="66"
					<%=allFields.getProperty("dependent_num").equals("66")?"selected":""%>>66</option>
			</select></td>
			<td class="bCellData">New Program Ind: <input type="text"
				name="newProgram" value="<%=allFields.getProperty("new_program")%>"
				size="2" maxlength="2" /></td>
		</tr>
		<tr>
			<td class="bCellData">After Hours: <select name="afterHours">
				<option value="0"
					<%=allFields.getProperty("after_hour").equals("0")?"selected":""%>>NO</option>
				<option value="E"
					<%=allFields.getProperty("after_hour").equals("E")?"selected":""%>>Evening
				Call</option>
				<option value="N"
					<%=allFields.getProperty("after_hour").equals("N")?"selected":""%>>Night
				Call</option>
				<option value="W"
					<%=allFields.getProperty("after_hour").equals("W")?"selected":""%>>Weekend
				Call</option>
			</select></td>
			<td class="bCellData">Time Call Recieved<!--TIME-CALL-RECVD-SRV-->
			<input type="text" name="timeCallRec"
				value="<%=allFields.getProperty("time_call")%>" size="4"
				maxlength="4" /> <input type="hidden" name="anatomicalArea"
				value="<%=allFields.getProperty("anatomical_area")%>" /></td>
		</tr>
		...
		<tr>
			...
			<input type="text" name="startTime"
				value="<%=allFields.getProperty("service_start_time")%>" size="4"
				maxlength="4" /></td>

			<td class="bCellData">Service Time Finish <%! /*SERVICE-TIME-FINISH*/ %>
			<input type="text" name="finishTime"
				value="<%=allFields.getProperty("service_end_time")%>" size="4"
				maxlength="4" /></td>

		</tr>
		<tr>
			<td class="bCellData">MVA <select name="mvaClaim">
				<option value="N"
					<%=allFields.getProperty("mva_claim_code").equals("N")?"selected":""%>>No</option>
				<option value="Y"
					<%=allFields.getProperty("mva_claim_code").equals("Y")?"selected":""%>>Yes</option>
			</select> <!--<input type="text" name="mvaClaim" value="<%=allFields.getProperty("mva_claim_code")%>" size="1"/>-->
			</td>
			<td class="bCellData">ICBC Claim Num: <input type="text"
				name="icbcClaim" value="<%=allFields.getProperty("icbc_claim_no")%>"
		...
		<tr>
			...
			<td class="bCellData">Facility Number <input type="text"
				name="facilityNum" value="<%=allFields.getProperty("facility_no")%>"
				size="5" maxlength="5" /></td>

			<td class="bCellData">Facility Sub Number <input type="text"
				name="facilitySubNum"
				value="<%=allFields.getProperty("facility_sub_no")%>" size="5"
				maxlength="5" /></td>
		</tr>
	</table>
	...
	<table width="100%" border=1>
		...
		<tr>
			<td class="bCellData"><input type="text" style="font-size: 80%;"
				name="service_code"
				value="<%=allFields.getProperty("billing_code")%>" size="10">
			<input type="button"
				onClick="javascript:popFeeItemList('ReProcessBilling','service_code');return false;"
				value="Search/Update" /></td>
			<td width="50%" class="bCellData"><%=billform.getServiceDesc(allFields.getProperty("billing_code"),billRegion)%>
			</td>
			<td class="bCellData"><input type="hidden" name="billing_unit"
				value="<%=allFields.getProperty("billing_unit")%>"> <input
				type="text" style="font-size: 80%;" name="billingUnit"
				value="<%=allFields.getProperty("billing_unit")%>" size="6"
				maxlength="6"></td>
			<td class="bCellData" nowrap>
			<div align="right"><input type="hidden" name="billing_amount"
				value="<%=allFields.getProperty("bill_amount")%>"> <input
				type="text" style="font-size: 80%;" size="8" maxlength="8"
				name="billingAmount"
				value="<%=allFields.getProperty("bill_amount")%>"
				onChange="javascript:validateNum(this)"></div>
			</td>
			<td><label> Amount: <input name="adjAmount" type="text"
				size="7" maxlength="7"> </label> <label> <input
   ...
   %>
		...
		<tr>
			...
			<table width="100%">
				<tr bgcolor="#CCCCFF">
					<td colspan=2 class="bCellData">Diagnostic Code</td>


				</tr>
				<tr>
					<td class="bCellData"><a href="javascript:ScriptAttach('dx1')">DX
					1</a><input type="text" name="dx1"
						value="<%=allFields.getProperty("dx_code1")%>" size="10">
					</td>
					<td><%=billform.getDiagDesc(allFields.getProperty("dx_code1"),billRegion)%></td>
				</tr>
				<tr>
					<td class="bCellData"><a href="javascript:ScriptAttach('dx2')">DX
					2</a><input type="text" name="dx2"
						value="<%=allFields.getProperty("dx_code2")%>" size="10">
					</td>
					<td><%=billform.getDiagDesc(allFields.getProperty("dx_code2"),billRegion)%></td>
				</tr>
				<tr>
					<td class="bCellData"><a href="javascript:ScriptAttach('dx3')">DX
					3</a><input type="text" name="dx3"
						value="<%=allFields.getProperty("dx_code3")%>" size="10">
					</td>
			...
			<table width="100%">
				<tr bgcolor="#CCCCFF">
					<td colspan=2 class="bCellData">Referrals <% String  refCD1 = allFields.getProperty("referral_flag1");
				...
				<tr>
					<td class="bCellData">1. <select name="referalPracCD1">
					...
					<td class="bCellData"><input type="button"
						onClick="javascript:ReferralScriptAttach('referalPrac1')"
						value="Search" /> <input type="text" name="referalPrac1"
						value="<%=allFields.getProperty("referral_no1")%>" size="5"
						maxlength="5" /></td>
				</tr>
				<tr>
					<td class="bCellData">2. <select name="referalPracCD2">
					...
					<td class="bCellData"><input type="button"
						onClick="javascript:ReferralScriptAttach('referalPrac2')"
						value="Search" /> <input type="text" name="referalPrac2"
						value="<%=allFields.getProperty("referral_no2")%>" size="5"
						maxlength="5" /></td>
				</tr>
			</table>
			</td>
		...
		<tr>
			<td class="bCellData">Payment Mode</td>
			<!--PAYMENT MODE-->
			<td class="bCellData"><select name="paymentMode">
				<option value="0"
					<%=allFields.getProperty("payment_mode").equals("0")?"selected":""%>>Fee
				For Service</option>
				<option value="E"
					<%=allFields.getProperty("payment_mode").equals("E")?"selected":""%>>Alternate
				Funding</option>
			</select></td>
			<td class="bCellData">Submission Code</td>
			<!--SUBMISSION-CODE-->
			<td class="bCellData"><select name="submissionCode"
				onChange="checkDebitRequest();">
				<option value="0"
					<%=allFields.getProperty("submission_code").equals("0")?"selected":""%>>0|Normal
				Submission</option>
				<option value="D"
					<%=allFields.getProperty("submission_code").equals("D")?"selected":""%>>D|Duplicate</option>
				<option value="E"
					<%=allFields.getProperty("submission_code").equals("E")?"selected":""%>>E|Debit
				Request</option>
				<option value="I"
					<%=allFields.getProperty("submission_code").equals("I")?"selected":""%>>I|ICBC
				Claim</option>
				<option value="W"
					<%=allFields.getProperty("submission_code").equals("W")?"selected":""%>>W|Claim
				not accepted by WCB</option>
				<option value="C"
					<%=allFields.getProperty("submission_code").equals("C")?"selected":""%>>C|Subscriber
				Coverage Problem</option>
				<option value="R"
					<%=allFields.getProperty("submission_code").equals("R")?"selected":""%>>R|Resubmit
				Claim</option>
				<option value="A"
					<%=allFields.getProperty("submission_code").equals("A")?"selected":""%>>A|Pre-approved
				claim</option>
				<option value="X"
					<%=allFields.getProperty("submission_code").equals("X")?"selected":""%>>X|Resubmitting
				refused or part paid</option>
			</select></td>
		</tr>
		<!--<tr>
       ...
       -->
		...
		<tr>
			...
			<div id="DEBITREQUEST">Select sequence number you would like to
			debit <input name="debitRequestSeqNum" type="text" maxlength="7"
				size="7"
				value="<%=getDebitRequestSeqNum(allFields.getProperty("original_claim"))%>" />

			</br>
			Select date MSP received claim (if not known, fill with zeros)
			(YYYYMMDD): <input id="debitRequestDate" name="debitRequestDate"
				type="text" maxlength="8" size="8"
				value="<%=getDebitRequestDate(allFields.getProperty("original_claim"))%>" />
			<a id="hlADate"><img title="Calendar"
				src="../../../images/cal.gif" alt="Calendar" border="0" /></a> <!--Date Received MSP <input type="text" />-->
			</div>
			</td>
		...
		<tr>
			...
			<td class="bCellData">Correspondence Code</td>
			<!--CORRESPONDENCE-CODE-->
			<td class="bCellData"><select name="correspondenceCode"
				onChange="correspondenceNote();">
				<option value="0"
					<%=allFields.getProperty("correspondence_code").equals("0")?"selected":""%>>None</option>
				<option value="C"
					<%=allFields.getProperty("correspondence_code").equals("C")?"selected":""%>>Paper
				Note</option>
				<option value="N"
					<%=allFields.getProperty("correspondence_code").equals("N")?"selected":""%>>Elec
				Note</option>
				<option value="B"
					<%=allFields.getProperty("correspondence_code").equals("B")?"selected":""%>>Both</option>
			</select> <!--<input type="text" name="correspondenceCode" value="<%=allFields.getProperty("correspondence_code")%>" size="1"/>-->
			</td>
			<td class="bCellData">Insurer Code</td>
			<!--OIN-INSURER-C0DE-->
			<td class="bCellData"><select name="insurerCode2">
				<option value=""
					<%=allFields.getProperty("oin_insurer_code").equals("0")?"selected":""%>>None</option>
				<option value="IN"
					<%=allFields.getProperty("oin_insurer_code").equals("C")?"selected":""%>>Institutional
				Claim</option>
				<option value="PP"
					<%=allFields.getProperty("oin_insurer_code").equals("C")?"selected":""%>>Pay
				Patient</option>
				<option value="WC"
					<%=allFields.getProperty("oin_insurer_code").equals("C")?"selected":""%>>WCB</option>
			</select></td>
		</tr>
		<tr>
			<td class="bCellData">Claim Short Comment</td>
			<!--CLAIM-SHORT-COMMENT-->
			<td><input type="text" name="shortComment"
				value="<%=allFields.getProperty("claim_comment")%>" size="20"
				maxlength="20" /></td>
			<td class="bCellData">Note</td>
			<td>
			<div id="CORRESPONDENCENOTE"><textarea cols="60" rows="5"
				name="notes" onKeyUp="checkTextLimit(this.form.notes,400);"><%=corrNote%></textarea>
			</div>
			</td>
			...
			<table width="100%">
				...
				<tr>
					...
					<td colspan="3"><textarea cols="60" rows="5"
						name="messageNotes"><%=messageNotes%>              </textarea></td>
					<td></td>
			</table>
			<td colspan="3"><jsp:include flush="false"
				page="billTransactions.jsp">
...
-->
 ...
 %>
	<tr>
		<td colspan="4" class="bCellData"><input type="submit"
			name="submit" value="Reprocess Bill"> <input type="submit"
			name="submit" value="Resubmit Bill"> <input type="submit"
			name="submit" value="Reprocess and Resubmit Bill"
			onClick="checkSubmitType()"> <input type="submit"
			name="submit" value="Settle Bill"></td>
	</tr>
	<%}else{%>
	<tr>
		<td colspan="4" class="bCellData"><input type="submit"
			name="submit" id="submitButton" style="display: none;"
			value="Reprocess and Resubmit Bill" onClick="checkSubmitType()">
		</td>
	</tr>
...
<a href="javascript: function myFunction() {return false; }"
	onClick="javascript: showRecord();">View Full Record</a>
...
<table border=1>
	...
	<tr>
		...
		<!--PRACTITIONER-NUM-->
		<td><%=allFields.getProperty("practitioner_no")%></td>
		<!--MSP PHN-->
		<td>5</td>
		<td></td>
	</tr>
	<tr>
		<td>PHN</td>
		<!--PRACTITIONER-NUM-->
		<td><%=allFields.getProperty("phn")%></td>
		<!--MSP PHN-->
		<td>10</td>
		<td>Y</td>
	</tr>
	...
	<tr>
		...
		<td>Service Clarification code</td>
	...
	<tr>
		...
		<td>Anatomical Area</td>
		<!--MSP SERVICE ANATOMICAL AREA-->
	...
	<tr>
		...
		<td>Address 1 WCB Date Of Injury</td>
		<!--OIN-ADDRESS-1		WCB DATE OF INJURY-->
	...
	<tr>
		...
		<td>Address 2 WCB AREA OF INJURY</td>
		<!--OIN-ADDRESS-2 WCB AREA OF INJURY ANATOMICAL-POSITION-->
	...
	<tr>
		...
		<Td>Address 3 WCB NATURE OF INJURY</td>
		<!--OIN-ADDRESS-3 WCB NATURE OF INJURY-->
	...
	<tr>
		...
		<td>Address 4 WCB Claim Number</td>
		<!--OIN-ADDRESS-4 WCB CLAIM NUMBER-->
