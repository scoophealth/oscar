...
<h3>----- DB Connection Pool Monitor -----
</h2>
<font color="red">Active Connections (borrowed from pool): <%=numActive%></font><br />
<font color="green">Idle Connections (currently in the pool): <%=numIdle%></font><br />
<br />

<hr>
<h3>----- Mysql Monitor -----</h3>
<table border="1"
	style="border-collapse: collapse; border: 1px solid grey; font-size: 12px;">
	<tr>
		<th>ConnID</th>
		<th>Usre</th>
		<th>Host</th>
...
Free Memory: <%=String.valueOf(Runtime.getRuntime().freeMemory()/1000000)%>
MB<br />
Total Memory: <%=String.valueOf(Runtime.getRuntime().totalMemory()/1000000)%>
MB<br />
Max Memory: <%=String.valueOf(Runtime.getRuntime().maxMemory()/1000000)%>
MB (Maximum memory JVM will attempt to use.)
...
<%
		...
		{
                    threadCount++;
			%> <a href="javascript: hideDiv('stacktrace<%=threadCount%>');">Hide
Stacktrace</a> | <a
	href="javascript: showDiv('stacktrace<%=threadCount%>');">Show FULL
Stacktrace</a><br />
<div id="stacktrace<%=threadCount%>"
	style="border: 1px solid grey; height: 70px; overflow: hidden;">
